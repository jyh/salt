...
/-!
# G-BANDROWS — the WIDE band-row bundle at op (A₃ band leg, deliverable 1)

The band mirror of `RowsWide.gold_box_rows_wide`.  Where the box leg reads its per-annulus geometry
off the LIVE gate `2^i < opZ N·pieceN kp + 1`, the high-pass band leg has NO liveness — it reads the
SAME geometry off the **y-floor** `opY N < pieceN kp` (BandClose's non-vanishing floor,
`2^{kp} > opY N ≥ N^{1/3}/2`, a factor `8` MORE room than the box's `N^{1/3}/16`).  Every residual
analytic row of the liveness-free engine `BandWin.gold_band_box_kerr` is discharged at this floor,
so the two-cutoff band price collapses onto the closed `boxPriceKerr Kc kp i`.  Only the carrier
rows (`hβ`/`hd1`/`hcop2`/`hDsetD`) remain — supplied, as in the box leg, by the assembly.

## What is NEW vs. `gold_box_rows_wide` (the y-floor swaps)

* **`hx` (`exp(10^{10}) ≤ goldCut N (ka+1)`)** — the box leg gets this from `gold_box_zx_rows`
  (via `hlive`); the band derives it from the boundary corner `2^i·2^{kp} ≤ goldCut(ka+1)` and the
  y-floor `opY N < 2^{kp}`, so `goldCut(ka+1) ≥ N^{1/3}/2`, hence `log goldCut ≥ log N/3 − log 2 ≥
  10^{10}` at the raised tower `log N ≥ 3·10^{10}` (`a12_log_ge`).
* **`hNfloor` (`goldCut^{1/3}/8 ≤ 2^{kp}`)** — from `goldCut ≤ N` and the y-floor
  `N^{1/3}/2 ≤ 2^{kp}`.
* **the `M`/`kp` floors** — `N^{1/3}/16 ≤ pieceM kp` and `N^{1/3}/16 ≤ 2^{kp}` come from the y-floor
  (`2^{kp} > opY N ≥ N^{1/3}/2`), NOT `gold_box_Mfloor`/`gold_kfloor_live_z` (which need `hlive`).
* **`hD1`** — `1 ≤ D` from the band floor `hDge : goldCut^{11/24}/8 ≤ D` (`goldCut^{11/24} ≥ 8`).

The D-INDEPENDENT rows (`hX2`/`hM2`/`hFX`/`hXsqrt`/`hMsqrt`/`herr_Mlev`/`hfloor`) and the six wide
D-upper rows (`herr_lev`/`hDscale`/`hDXM`/`hDx`/`hDsq`/`habs`) are discharged exactly as in
`gold_box_rows_wide` (their routes never read the liveness — only `hXge`/`hMfloor`/`h2kpge` and the
wide hypothesis `hDwide`, all of which the y-floor supplies).
...
`gold_const_le_rpow`. -/
private theorem gold_const_le_rpow_bw {N : ℕ} (hNpos : 0 < (N : ℝ))
...
private theorem gold_Lpow_le_bw {N : ℕ} {L : ℝ} (hlogN1 : 1 ≤ Real.log N)
...
/-! ## 1. The wide-window band-box price at the y-floor -/

set_option maxHeartbeats 4000000 in
-- The ~24-row wide-geometry discharge elaborates a long rpow/sqrt-bearing chain and applies the
-- 39-hypothesis `gold_band_box_kerr`; the whnf/defeq checks need headroom above the default.
/-- **`gold_band_wide_price_at_op`.**  At a non-vanishing high-pass band survivor at op (the y-floor
`opY N < pieceN kp`) with a per-annulus level in the WIDE window (`hDge : goldCut^{11/24}/8 ≤ D`,
`hDwide : D·L^{18} ≤ √(X·M)`, `L := log((2^{i+1}−1)·pieceM kp)`), the two-cutoff band-box price is
`≤ boxPriceKerr Kc kp i` — every residual analytic row of `gold_band_box_kerr` discharged at the
y-floor.  Instantiable at `D := opD N` on the high annuli the band populates.  Only the
carrier/`Dset` rows (`hβ`/`hd1`/`hcop2`/`hDsetD`) remain, supplied by the assembly.  This is `hbox`
residual and — chained through `gold_band_survsum_geoN_floored` — Surv's `hwide`. -/
theorem gold_band_wide_price_at_op : ∃ (Kc : ℝ) (x₁ : ℕ), 0 < Kc ∧
    ∀ (N : ℕ), x₁ ≤ N →
  ...
  obtain ⟨Kc, N₀, hKc, hbody⟩ := gold_band_box_kerr
  obtain ⟨xs, hs⟩ := gold_op_scales
  obtain ⟨xp, hp⟩ := a12_logpow_le_rpow 18 (1 / 2000) (by norm_num) (by norm_num)
  obtain ⟨xh, hh⟩ := a12_log_ge (3 * 10 ^ 10 + 3)
  refine ⟨Kc, max (max (max xs xp) xh) ((16 * N₀) ^ 3 + 1), hKc, ?_⟩
  intro N hN β kp D Dset r T₁ T₂ ka i K hi hyfloor hDge hDwide hβ hd1 hcop2 hDsetD
  have hxs : xs ≤ N := le_trans (le_trans (le_trans (le_max_left _ _) (le_max_left _ _))
    (le_max_left _ _)) hN
  have hxp : xp ≤ N := le_trans (le_trans (le_trans (le_max_right _ _) (le_max_left _ _))
    (le_max_left _ _)) hN
  have hxh : xh ≤ N := le_trans (le_trans (le_max_right _ _) (le_max_left _ _)) hN
  ...
  -- =============== the y-floor geometry (NEW vs. the box leg) ===============
  -- the y-floor `opY N < 2^kp`, and the corner `2^kp ≤ goldCut(ka+1)`.
  have hyfloor' : opY N < 2 ^ kp := by unfold pieceN at hyfloor; omega
  have hN13nn : (0 : ℝ) ≤ (N : ℝ) ^ ((1 : ℝ) / 3) := Real.rpow_nonneg hNpos.le _
  have h2kpstrong : (N : ℝ) ^ ((1 : ℝ) / 3) / 2 ≤ ((2 ^ kp : ℕ) : ℝ) := by
    have h1 : (opY N : ℝ) < ((2 ^ kp : ℕ) : ℝ) := by exact_mod_cast hyfloor'
    linarith [hYhalf, h1]
  have h2kpge : (N : ℝ) ^ ((1 : ℝ) / 3) / 16 ≤ ((2 ^ kp : ℕ) : ℝ) := by
    linarith [h2kpstrong, hN13nn]
  ...
  -- the corner `2^kp ≤ goldCut(ka+1)`, so `goldCut(ka+1) ≥ N^{1/3}/2`.
  have hcopy := hi
  ...
  -- `hNfloor : goldCut^{1/3}/8 ≤ 2^kp`.
  have hNfloor : (goldCut N (ka + 1) : ℝ) ^ ((1 : ℝ) / 3) / 8 ≤ ((2 ^ kp : ℕ) : ℝ) := by
    ...
    linarith [hgc13, h2kpstrong, hN13nn]
  -- `hk : 2 ≤ kp`
  ...
  have hXsqrt : L ^ ((13 : ℝ) + 3) ≤ Real.sqrt ((2 ^ (i + 1) - 1 : ℕ) : ℝ) := by
    have hLpow : L ^ ((13 : ℝ) + 3) ≤ 262144 * (N : ℝ) ^ ((1 : ℝ) / 2000) :=
      gold_Lpow_le_bw hlogN1 hL0 hLle hpl (by norm_num) (by norm_num)
    have htail : (262144 : ℝ) * (N : ℝ) ^ ((1 : ℝ) / 2000) ≤ (N : ℝ) ^ ((11 : ℝ) / 48) / 2 := by
      have hm := gold_mono_close_bw hNpos hlogN9 (a := 1 / 2000) (b := 11 / 48) (C := 524288)
  ...
  have hMrow : ∀ E : ℝ, 0 ≤ E → E ≤ 18 → L ^ E ≤ Real.sqrt (pieceM kp : ℝ) := by
    ...
    have hLpow : L ^ E ≤ 262144 * (N : ℝ) ^ ((1 : ℝ) / 2000) :=
      gold_Lpow_le_bw hlogN1 hL0 hLle hpl hE0 hE18
    have htail : (262144 : ℝ) * (N : ℝ) ^ ((1 : ℝ) / 2000) ≤ (N : ℝ) ^ ((1 : ℝ) / 6) / 4 := by
      have hm := gold_mono_close_bw hNpos hlogN9 (a := 1 / 2000) (b := 1 / 6) (C := 1048576)
  ...
  have hDsq : D < (2 ^ kp + 1) * (2 ^ kp + 1) := by
    ...
    have h2N_le : 2 * (N : ℝ) ≤ ((N : ℝ) ^ ((2 : ℝ) / 3) / 256)
      ...
      have hstep : (134217728 : ℝ) * (N : ℝ) ^ (1 : ℝ) ≤ (N : ℝ) ^ ((4 : ℝ) / 3) := by
        have := gold_mono_close_bw hNpos hlogN9 (a := 1) (b := 4 / 3) (C := 134217728)
  ...
  have hD1 : 1 ≤ D := by
    have hgcpow_pos : (0 : ℝ) < (goldCut N (ka + 1) : ℝ) ^ ((11 : ℝ) / 24) :=
      Real.rpow_pos_of_pos hgcPos _
    have hge8 : (8 : ℝ) ≤ (goldCut N (ka + 1) : ℝ) ^ ((11 : ℝ) / 24) := by
    ...
    have hDreal : (1 : ℝ) ≤ (D : ℝ) := by linarith [hDge, hge8]
      ...
      have hBBlo : 2048 * (N : ℝ)
        ...
        have h2N_le : 2048 * (N : ℝ) ≤ ((N : ℝ) ^ ((2 : ℝ) / 3) / 256)
          ...
          have hstep : (137438953472 : ℝ) * (N : ℝ) ^ (1 : ℝ) ≤ (N : ℝ) ^ ((4 : ℝ) / 3) := by
            have := gold_mono_close_bw hNpos hlogN9 (a := 1) (b := 4 / 3) (C := 137438953472)
  ...
  -- close via `gold_band_box_kerr` at the op values `x := N`, `Lb := log(4N)`, `F := opZ N·opY N`
  exact hbody (N : ℝ) (Real.log (4 * (N : ℝ))) (opZ N * opY N) β kp D Dset r T₁ T₂ N (opZ N)
    (opY N) K ka i hN2 hi hx hNfloor hDge hk hβ hN₀ hd1 hcop2 hL1 hD1 hDsetD hDsq habs
    hX2 hM2 hDscale hXsqrt hMsqrt herr_lev herr_Mlev hFX hDx hLbb hfloor herr_LEpos hDXM herr_scale
