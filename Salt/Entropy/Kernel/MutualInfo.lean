...
(PFR) project, upstream file `PFR/ForMathlib/Entropy/Kernel/MutualInfo.lean`, at
...
- Converted the PFR module-system header (`module`; the `public import`s of
  `PFR.Mathlib.Probability.Kernel.Composition.Comp` and
  `PFR.ForMathlib.Entropy.Kernel.Basic`; and `@[expose] public section`) to a
  plain `import Mathlib` plus imports of the Salt ports of those two upstream
  modules, `Salt.Entropy.Mathlib.KernelComp` and `Salt.Entropy.Kernel.Basic`.
- All declarations, names, notation, namespaces, and proof bodies are verbatim
  from upstream; drift v4.33-rc1 → v4.32.0-rc1 required no statement or proof
