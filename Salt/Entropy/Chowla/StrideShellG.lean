...
/-! ## ⟦β W1 E1⟧ — THE AFFINE HEAD AT THE `2^12` CROWN, CAP 9 (build freeze v2 v1.1, 2026-09-13)

Additive only — both declarations above are untouched.  Each twin is its source's statement and body
with ONLY the raises `2 ^ 11 ↦ 2 ^ 12` (the door grade) and `log (a·h) ≤ 7 ↦ ≤ 9` (the product
cap); no hypothesis is added and no conclusion weakened. -/

set_option maxHeartbeats 1600000 in
-- THE HEAD is one large elaboration (the crown's payload, the five-wide reduce leaf and the circle
-- slot all in context while the pin's numerals and the budget witness are discharged); the ceiling
-- is the landed head's own (`StrideShell.lean:397`, 1600000) — the body is that head's, verbatim.
/-- **⟦β W1 E1⟧ F5-β-S3 at `2^12` and cap 9 — `log_chowla_aff_of_door_g12b`.**
`log_chowla_aff_of_door_g` with the grade `837782 * 2 ^ 11 ↦ 837782 * 2 ^ 12` in EXACTLY its two
places (the `hcrown` binder and the conclusion) and its UNUSED product-cap binder
`_hah7 : log (a·h) ≤ 7 ↦ _hah9 : ≤ 9` (census band 4 row 12: TRANSPORT — `hCnum`, `hepsa`, `hkey`,
`hcountR` are cap-free).  BODY: the source's VERBATIM — `hρle` is obtained from `hcrown A₀'` and
re-packaged untouched; the `maxHeartbeats 1600000` is the source's. -/
theorem log_chowla_aff_of_door_g12b (a b h : ℕ) (ha : 0 < a) (hh : 0 < h) (hba : b < a)
    (_hah9 : Real.log ((a * h : ℕ) : ℝ) ≤ 9)
    (hcm : ∃ C : ℝ, 0 < C ∧ C ≤ (h : ℝ) * (1 + 2 * (2 * Real.log 4)) ∧
      ∀ (eps : ℚ) (H : ℕ) [NeZero H] (x1 : Fin H → ℤ),
      (∀ i, |x1 i| ≤ 1) →
      a ∣ H →
...
/-! ## ⟦β W1 E1⟧ F5-β-N11 at `2^12` — the graded head without its slot -/

/-- **⟦β W1 E1⟧ F5-β-N11 at `2^12` and cap 9 — the graded affine head UNSLOTTED.**
`log_chowla_aff_of_door_unslotted_g` with `837782 * 2 ^ 11 ↦ 837782 * 2 ^ 12` in `hcrown` and the
conclusion and `hah7 ↦ hah9 : log (a·h) ≤ 9`; the body is the source's one-liner at
`log_chowla_aff_of_door_g12b` (census band 4 row 13: SUPPLIER-SWAP). -/
theorem log_chowla_aff_of_door_unslotted_g12b (a b h : ℕ) (ha : 0 < a) (hh : 0 < h) (hba : b < a)
    (hgcd : Nat.gcd (b + h) a ∣ h)
    ...
    ∃ (ε : ℚ) (A : ℝ), 0 < ε ∧ ε = 1 / (500 * ((a * h : ℕ) : ℚ)) ∧ 162 ≤ A ∧ A₀ ≤ A ∧
      ∃ Ra : ChowlaRegimeAff, Ra.a = a ∧ Ra.b = b ∧ Ra.eps = ε ∧
        ...
        ∃ δ₀ : ℝ, 0 < δ₀ ∧ 1 / (838400 * ((a * h : ℕ) : ℝ) ^ 2) ≤ δ₀ ∧
          ∀ ρ' : ℝ, 0 < ρ' → ρ' ≤ δ₀ → MRTUniformityXiL2AffW h Ra ρ' →
            ¬ logChowlaFailsAff a b h Ra.eps Ra.x Ra.ω := by
  exact log_chowla_aff_of_door_g12b a b h ha hh hba hah9
    (circle_method_estimate_sq_bounded_aff a b h ha hh hgcd (2 * Real.log 4) (by positivity))
    hcrown A₀
