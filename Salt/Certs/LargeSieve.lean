...
/-- ⛔ **RULE 6 — VACUITY CONTROL for the analytic form.** The spacing hypothesis must be
shown SATISFIABLE, and **non-vacuously**: at `R = 1` the condition `∀ r s, r ≠ s → …` holds
because there is no pair to test, which would certify nothing. *The witness therefore uses
`R = 2`: two genuinely separated points `{0, 1/2}` at the extreme admissible `δ = 1/2`.* -/
...
/-- **RULE 6 for the character form.** Its single hypothesis is `2 ≤ Q`, satisfied at
`Q = 2`. *Recorded even though it is trivial: the rule is that a hypothesis-carrying cert
lands with a witness, and "obviously satisfiable" is the judgement the rule exists to
replace.* -/
