...
/-- **N5.2, general exponent.** Same as `eh_error_small` but with the EH power
`A = 9k² + 2B` instead of `9k² + 4`, giving the `(3k)^{ω(q)}`-weighted
discrepancy sum `≤ C · N / (log N)^B` for arbitrary `B ≥ 1` (the S₂ error uses
`B = 2k + 4` to dominate `λ²_max ~ (log N)^{2k+2}`). The Cauchy–Schwarz square
root halves the EH `2B` back to `B`. `C = √(2·CA·C₁^{9k²})` with `CA` the `EH`
constant at level `A = 9k² + 2B`. -/
theorem eh_error_pow (k B : ℕ) (hB : 1 ≤ B) (hEH : EH (1 / 2)) :
    ∃ (C : ℝ) (N₀ : ℕ), ∀ N : ℕ, N₀ ≤ N →
      ∑ q ∈ (Finset.range (⌊(N : ℝ) ^ (1 / 2 : ℝ)⌋₊ + 1)).filter Squarefree,
          ((3 * k : ℝ) ^ q.primeFactors.card * maxDiscrepancy N q)
        ≤ C * (N : ℝ) / (Real.log N) ^ B := by
  obtain ⟨C₁, hC₁1, hC₁⟩ := rankin_bound (9 * k ^ 2)
  obtain ⟨CA, hCA⟩ := hEH ((9 * k ^ 2 + 2 * B : ℕ) : ℝ) (by
  ...
  have hLHSsq : LHS ^ 2
    ...
    rw [hfg, hff, hgg] at hcs
    exact hcs
  -- ===== EH factor: Σ D ≤ CA · N / (log N)^{9k²+2B} =====
  have hsub : S ⊆ Finset.Icc 1 qmax := by
    intro q hq
    rw [hSdef, Finset.mem_filter, Finset.mem_range] at hq
    obtain ⟨hqlt, hqsf⟩ := hq
  ...
  have hE : (∑ q ∈ S, maxDiscrepancy N q)
    ...
    calc (∑ q ∈ S, maxDiscrepancy N q)
        ≤ ∑ q ∈ Finset.Icc 1 qmax, maxDiscrepancy N q :=
          Finset.sum_le_sum_of_subset_of_nonneg hsub (fun q _ _ => maxDiscrepancy_nonneg N q)
      _ ≤ CA * (N : ℝ) / (Real.log N) ^ ((9 * k ^ 2 + 2 * B : ℕ) : ℝ) := hCA N hN2
  ...
  have hlogNne : Real.log N ≠ 0 := ne_of_gt hlogN
  have h1ne : (Real.log N) ^ (9 * k ^ 2) ≠ 0 := pow_ne_zero _ hlogNne
  have h4ne : (Real.log N) ^ (2 * B) ≠ 0 := pow_ne_zero _ hlogNne
