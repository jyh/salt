...
/-!
# ⟦R3-A — THE CAPSTONE AT THE FREE POOL⟧ (`M4MeanSqPool`)

⟦R2⟧'s deferred item, in the crossing lane: `M4MeanSq`'s two ∃-restatements re-stated at
`ThmA2Pool.thm_a2'_of_rows_pool`'s gate list.  Provenance: `flags.md` 2026-07-30 17:24
(⟦R2⟧, "DEFERRED: the M4MeanSq twins") and MAESTRO ERRATUM #2 on the same entry.

## ⟦WHAT MOVES — FOUR LINES OF THE BINDER LIST⟧

Against `M4MeanSq.m4_meansq_per_chi_gen`, byte for byte except:

* the real binder block gains `π₀`;
* `ε ≤ θ₂₉₃ − 1/500` is **DROPPED** — ⟦R2⟧'s gift: the `𝒰`-leg's exponent room is no longer
  an interface hypothesis;
* the two grading gates read `≤ π₀`, and `hL4096` is replaced by the two STATED gates
  `hgU : (log X)^{−θ₂₉₃+ε} ≤ π₀` and `hgBand : 4096·(log X)^{−1+1/500} ≤ π₀`, plus
  `hpool : 0 ≤ π₀`;
* the conclusion's third summand is `188133·π₀`.

## ⚠ ⟦MAESTRO ERRATUM #2, HONOURED⟧ — `habs` SURVIVES

`8640 ≤ (log X)^ε` is **carried VERBATIM**.  It is a `𝒰`-LEG gate (the `EP2` budget's
absorption exponent), not a pool gate: the pool frees the exponent ROOM `ε ≤ θ₂₉₃ − 1/500`,
it does not free `ε` from the leg's own demand.  Conflating the two was the erratum; the
honest resolution named there (THE εr/ε SPLIT — the crossing's exponent and the cap bundle's
absorption exponent are different roles fed one symbol) is a separate binder change in the
perBlock capstone and is NOT performed here.  Every other `ε`-reading binder
(`12·EP2 ≤ (log X)^{−θ₂₉₃+ε}`, `0 ≤ ε`) is likewise verbatim.

## Contents

* §1 `m4_meansq_per_chi_gen_pool`;
* §2 `m4_meansq_or_trivial_pool`.

Additive: no landed declaration is touched.  Setting `π₀ := (log X)^{−1/500}` recovers the
landed statements (with `hgU`/`hgBand` then derivable from `hεwin`/`hL4096`, which is exactly
what `ThmA2Prime.thm_a2'_of_rows'` records at the interface).
-/
...
/-! ## §1 — THE FIVE-SUMMAND MEAN SQUARE AT THE POOL -/
...
theorem m4_meansq_per_chi_gen_pool :
...
/-! ## §2 — THE PER-SCALE DICHOTOMY AT THE POOL -/
...
theorem m4_meansq_or_trivial_pool (Qm : ℕ) :
...
The pooled M4 mean-square pair at `G := s13GK K M` (`GLever`).  Statements are the landed
ones with `3072 * M` replaced by `s13GK K M`; the suppliers are
`ThmA2Rows.a2Rows_of_capfree3_gk` and `ThmA2Pool.thm_a2'_of_rows_pool_gk`.  Level-1 objects
keep their landed names; the side condition `K ≤ 1.7·10⁸` is the frame's. -/

set_option maxHeartbeats 1600000 in
theorem m4_meansq_per_chi_gen_pool_gk (K : ℕ) (hK : K ≤ 170000000) :
    ∃ (Cq cq T₀ X₀ Cs Ccc : ℝ) (Kfl : ℕ → ℝ),
      0 < Cq ∧ 0 < cq ∧ 3 ≤ T₀ ∧ 0 < X₀ ∧ 0 < Cs ∧ 0 < Ccc ∧ (∀ Qm : ℕ, 0 ≤ Kfl Qm) ∧
      ∀ (Qm q : ℕ) [NeZero q] (_χ : DirichletCharacter ℂ q), q ≤ Qm →
          ∀ (N Xd P Q M : ℕ) (a cf b : ℕ → ℂ) (bfam : ℕ → ℕ → ℂ)
  ...
  have hrows := hrow cf a b cf bfam
    ...
    hcoefBand hQXd hXdbig hN4 hdom ha1 hasupp
  -- ⟦THE FROZEN INTERFACE⟧
  exact thm_a2'_of_rows_pool_gk K hM hXe hX3 hh4 hhX ha1 hsupp0 hN2X hTann hceil5 hrows hT0band
    hpool hgP1 hgRows hgU hgBand

set_option maxHeartbeats 1600000 in
theorem m4_meansq_or_trivial_pool_gk (K : ℕ) (hK : K ≤ 170000000) (Qm : ℕ) :
    ∃ Cq cq T₀ X₀ Cs Ccc Kfl : ℝ,
      0 < Cq ∧ 0 < cq ∧ 3 ≤ T₀ ∧ 0 < X₀ ∧ 0 < Cs ∧ 0 < Ccc ∧ 0 ≤ Kfl ∧
      ∀ (q : ℕ) [NeZero q] (_χ : DirichletCharacter ℂ q), q ≤ Qm →
          ∀ (N Xd P Q M : ℕ) (a cf b : ℕ → ℂ) (bfam : ℕ → ℕ → ℂ)
