...
with ONLY the freeze's §3.1 rule-2 raises (`c ≤ 1096 ↦ ≤ 8103`, `1201216 ↦ 65658609`), every derived
cap-dependent supplier replaced by its twin; no hypothesis is added and no conclusion weakened. -/

set_option maxHeartbeats 1600000 in
-- as the source: eleven register lines at a SYMBOLIC design point, `blk` a `y⁴`-vs-`e^y` comparison
/-- `s15_sel''_L_witness_flat` at the cap-9 shift bound (`s15_sel''_L_witness_flat_b9`) —
NUMERAL-LIFT: `hcb : c ≤ 1096 ↦ c ≤ 8103` and `c² ≤ 1201216 ↦ c² ≤ 65658609` in `hcsqb`.  The shift
enters only in the `blk` line, as `1006632960·c² ≤ 6.61·10^16` against the literal
`E² ≥ 1.209·10^18` (census band 2 row 2: ×18.3).  Every other step is the source's, verbatim. -/
theorem s15_sel''_L_witness_flat_b9 {A : ℝ} (hA : 26 ≤ A) {Cg δ₀ Ct K : ℝ} {x₀ Mfl c : ℕ}
    {R : ChowlaRegime}
    (hc1 : 1 ≤ c) (hcb : c ≤ 8103)
    (hδ : 0 < δ₀) (hδb : 1 / 2 ^ 10 ≤ δ₀)
    (hK : 0 < K) (hKb : K ≤ 2 ^ 20)
  ...
  · have hbeN : s13BlockExp_L (flatDoorM A) ≤ 2 ^ 104 * (flatDoorM A) ^ 6 :=
    ...
    have hcR1 : (1 : ℝ) ≤ (c : ℝ) := by exact_mod_cast hc1
    have hcR0 : (0 : ℝ) < (c : ℝ) := by linarith
    have hcRb : (c : ℝ) ≤ 8103 := by exact_mod_cast hcb
    have hepsR : (1 : ℝ) / (512 * (c : ℝ)) ≤ (R.eps : ℝ) := by
      have hcast : (((1 : ℚ) / (2 ^ 9 * (c : ℚ)) : ℚ) : ℝ) ≤ ((R.eps : ℚ) : ℝ) :=
        (Rat.cast_le (K := ℝ)).mpr heps
      have heq : (((1 : ℚ) / (2 ^ 9 * (c : ℚ)) : ℚ) : ℝ) = 1 / (512 * (c : ℝ)) := by
    ...
    have hbig : 15 * E ^ (6 : ℕ) ≤ 1 / (262144 * (c : ℝ) ^ 2) * Real.exp (E ^ 2) := by
      have hcsq : (1 : ℝ) ≤ (c : ℝ) ^ 2 := by nlinarith [hcR1]
      have hcsqb : (c : ℝ) ^ 2 ≤ 65658609 := by nlinarith [hcR1, hcRb]
      have h2 : (1 : ℝ) / (262144 * (c : ℝ) ^ 2) * ((E ^ 2) ^ 4 / 256)
          = E ^ (6 : ℕ) * E ^ (2 : ℕ) / (67108864 * (c : ℝ) ^ 2) := by field_simp; ring
      have h3 : (15 : ℝ) * E ^ (6 : ℕ)
          ≤ E ^ (6 : ℕ) * E ^ (2 : ℕ) / (67108864 * (c : ℝ) ^ 2) := by
...
/-- `s15_sel''_L_gk_witness_flat` at the cap-9 shift bound (`s15_sel''_L_gk_witness_flat_b9`) —
NUMERAL-LIFT (`c ≤ 8103`, `c² ≤ 65658609`) + SUPPLIER-SWAP (`s15_sel''_L_witness_flat_b9`).  The
lever's `blk` line spends `2·262144·c² = 3.44·10^13` against `exp 40 ≥ 10^17` (census band 2
row 1).  Every other step is the source's, verbatim. -/
theorem s15_sel''_L_gk_witness_flat_b9 {A : ℝ} (hA : 26 ≤ A) (Klev : ℕ)
    (hKle : Klev ≤ 170000000 * flatDoorM A) {Cg δ₀ Ct K : ℝ} {x₀ Mfl c : ℕ}
    (hc1 : 1 ≤ c) (hcb : c ≤ 8103)
    {R : ChowlaRegime}
    (hδ : 0 < δ₀) (hδb : 1 / 2 ^ 10 ≤ δ₀)
    ...
    S15Sel''_L_gk Klev Cg δ₀ Ct (doorRhoOfDelta (s12DeltaSock δ₀ K)) x₀ Mfl R
      (flatDoorM A) := by
  refine s15_sel''_L_gk_of_L Klev
    (s15_sel''_L_witness_flat_b9 hA hc1 hcb hδ hδb hK hKb hCt hCtb hbfl hMfl hx0win heps hlo hhi)
    ?_
  set E : ℝ := Real.exp (3.2 * A / 2) with hEdef
  set Mr : ℝ := ((flatDoorM A : ℕ) : ℝ) with hMrdef
  have hE17 : (10 : ℝ) ^ 17 ≤ E := flat_exp_half_ge hA
