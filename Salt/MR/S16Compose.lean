...
theorem logChowla2_witnessed_scale_flat_L_v2_uniform_win_ceiling (Awin : ℝ)
  ...
  have hsel := s15_sel''_L_gk_witness_flat_bumped_win (c := 1) hA26 32000000 hKle (by norm_num) (by norm_num) (by simp) hδ₀ (by simpa using hδpin) hKc hKcb
...
/-- ⟦WIDE CEILING TWIN⟧ (`m4_closure_fuse_zero'_const_nonneg_L_gk_ceiling_kwide`) —
`m4_closure_fuse_zero'_const_nonneg_L_gk_ceiling`
with the flat ceiling moved inside the `∀ M` as `K ≤ 170000000 * M`.
Statement and proof otherwise verbatim, off the `_kwide` upstream. -/
theorem m4_closure_fuse_zero'_const_nonneg_L_gk_ceiling_kwide (K : ℕ) :
    ∃ Ct : ℝ, 0 < Ct ∧ Ct ≤ 2 ^ 23 ∧
      ∀ (Cp : ℝ), 0 ≤ Cp →
      ∀ (R : ChowlaRegime) (M : ℕ) (C₁ M₀ ε : ℕ → ℝ) (Kc ρ : ℝ)
        (cU : ℕ → ℂ) (bU : ℕ → ℕ → ℂ) (t₁ : ∀ q : ℕ, DirichletCharacter ℂ q → ℝ),
        ...
        (∀ H L q j A s : ℕ, SocketBaseL R M H L q j A s →
          DoorArithFrameRho_L M H j (((A + s : ℕ)) : ℝ) (C₁ (A + s)) (M₀ (A + s)) Kc ρ) →
        M4ChiSummedFreeRow_L_gk K R M (m4ChiRowGraded_L M (fun _ H => RSanDoorRho ρ H)) := by
  obtain ⟨Ct, hCt, hCtb, hslot⟩ := m4_hrowsSlot_at_door_zero'_L_gk_ceiling_kwide K
  refine ⟨Ct, hCt, hCtb, ?_⟩
...
/-- ⟦WIDE CEILING TWIN⟧ (`flat_capstone_uniform_win_ceiling_kwide`) —
`flat_capstone_uniform_win_ceiling`
with the flat ceiling moved inside the `∀ M` as `K ≤ 170000000 * M`.
Statement and proof otherwise verbatim, off the `_kwide` upstream. -/
theorem flat_capstone_uniform_win_ceiling_kwide (K : ℕ) (Awin : ℝ)
    (hband : S16BandLaneCBoundedL_win Awin K) :
    ∃ (Cg : ℝ) (ε : ℚ) (Kc δ₀ Ct Cq cs T₀ Kq Ks β : ℝ) (x₀ Hopq Mfl : ℕ),
      1 ≤ Cg ∧ 0 < ε ∧ 0 < Kc ∧ 0 < δ₀ ∧
        0 < Ct ∧ 0 < Cq ∧ 0 < cs ∧ 3 ≤ T₀ ∧ 0 < Kq ∧ 0 < Ks ∧ 1 ≤ Mfl ∧
...
/-- ⟦WIDE CEILING TWIN⟧ (`flat_conditional_uniform_win_ceiling_kwide`) —
`flat_conditional_uniform_win_ceiling`
with the flat ceiling moved inside the `∀ M` as `K ≤ 170000000 * M`.
Statement and proof otherwise verbatim, off the `_kwide` upstream. -/
theorem flat_conditional_uniform_win_ceiling_kwide (K : ℕ) (Awin : ℝ)
    (hband : S16BandLaneCBoundedL_win Awin K) :
    ∃ (ε : ℚ) (Cg Kc δ₀ Ct β : ℝ) (x₀ Hopq Mfl : ℕ),
      0 < ε ∧ 1 ≤ Cg ∧ 0 < Kc ∧ 0 < δ₀ ∧ 0 < Ct ∧ 1 ≤ Mfl ∧
      Cg ≤ 2 * 10 ^ 12 ∧ 1 / 500 ≤ ε ∧ 1 / 838400 ≤ δ₀ ∧
  ...
  obtain ⟨Cg, ε, Kc, δ₀, Ct, Cq, cs, T₀, Kq, Ks, β, x₀, Hopq, Mfl, hCg, hε, hKc,
    hδ₀, hCt, hCq, hcs, hT₀, hKq, hKs, hMfl, hCgle, hεpin, hδpin, hKcb, hCtb, hMflb,
    hβ, hcapU⟩ :=
    flat_capstone_uniform_win_ceiling_kwide K Awin hband
  refine ⟨ε, Cg, Kc, δ₀, Ct, β, x₀, Hopq, Mfl, hε, hCg, hKc, hδ₀, hCt, hMfl,
    hCgle, hεpin, hδpin, hKcb, hCtb, hMflb, hβ, ?_⟩
  intro A hA26 hAge
  obtain ⟨Hcap, hCapLe, hmain⟩ := hcapU A hA26 hAge
