...
3. **the x-scale antecedent** `R.x ≤ 16·R.ω·arcDen 12 H·A` — i.e. `A ≳ x/(16·ω·(log H)^{12})`,
   ...
   consumer performs;
4. **the base cap** `A ≤ 2·R.x` — the (α) base-cap surgery, JYH-granted 2026-07-30; the
   ⟦THE (α) BASE-CAP SURGERY⟧ block below states the defect it closes and the read
   verification.
...
## ⟦THE (α) BASE-CAP SURGERY⟧ — **JYH-granted 2026-07-30**, the FOURTH base antecedent

⟦THE DEFECT⟧ `M4Assembly.DoorFuseFrame.gP1` and the `X_d`-dependent fields of `gRows` cap
the fused base `X_d = A + s` from ABOVE (they are `(log X_d)^{-1/500}`-comparisons, and that
right-hand side DECAYS).  The socket's `∀ A` was unbounded above and closed upward, so
`M4Assembly.SocketBase` was inhabited arbitrarily far up and every `∀`-base frame hypothesis
was refutable there (the refuter's kernel kill `hframe_unsatisfiable`, `flags.md` 2026-07-30
08:47).  The frame was not wrong; its quantifier range was.

⟦THE REPAIR⟧ the socket's own `∀`-prefix gains `(A : ℝ) ≤ 2·R.x`.  This WEAKENS the socket
(a supplier serves fewer bases; every consumer must now produce the cap at its read), which
is why it is stated here and discharged at the ONE place the base becomes concrete.

⟦THE CONSUMER-READ VERIFICATION, AT THE BYTES⟧ the socket has exactly one consumption
chain, and the base is a free variable at every link until the last:

* `m4_chiSummedShiftBlock_of_freeRow` reads the socket at the SAME `A` its own conclusion
  binds (§2), so the cap propagates verbatim into `M4ChiSummedFreeShiftBlock`;
* `m4_chiSummedBlockN_of_shiftBlock` applies `hfix` at the SAME `A` (§3, `hjtL`), so it
  propagates verbatim into `M4ChiSummedBlockMeanSqN`;
* `M4Gauss.m4_freeBlockSup_of_chiSummed` reads it at the DILATED base `⌊A/d⌋ − 1 ≤ A`, so
  the cap is inherited by monotonicity — no `arcDen` power is spent on it (unlike the three
  lower antecedents, which each cost one);
* `M4SecondRoad.m4_blockMeanSqBlk2_of_chiSummed` is where the base becomes CONCRETE: the
  block bottom is `X_{i+1} + m·ℓ` with `X_· = doorLadder R.x H ·`, `i + 1 ≥ 1` and
  `m·ℓ ≤ H`.  The ladder is antitone above its floor and `H + 1 ≤ R.x`
  (`regime_window_headroom`), so
  `X_{i+1} ≤ X_1 = ⌊(R.x + H + 1)/2⌋ ≤ R.x` and `X_{i+1} + m·ℓ ≤ R.x + H ≤ 2·R.x − 1`.

So `2·R.x` is the honest cap: every read satisfies it, with one unit to spare, and no read
satisfies anything smaller (the top rung `X_1` is `≈ R.x` and the drift shift is `≈ H`).
`R.x` does NOT become a register conjunct — the cap lives inside the socket's `∀`-prefix,
exactly like the x-scale antecedent, and the ⟦F5 CHECK⟧ is unchanged: read as an
`X`-comparison it is an `X`-LOWER *given to* the supplier, never an `X`-upper on the regime.
...
⟦the (α) base-cap surgery, JYH-granted 2026-07-30⟧ the base cap rides here too, so the
bridge stays byte-faithful to the socket's `∀`-prefix. -/
theorem chiFreeRowSq_le_of_chiSummed {R : ChowlaRegime} {M : ℕ} {RS : ℕ → ℕ → ℝ}
  ...
  intro H hlo hhi L hLH q hq hqQ χ j hjL A hA hAj hAsq hAx hAcap s hsL
  have hAs : 0 < A + s := by omega
  ...
  exact le_trans hle (h H hlo hhi L hLH q hq hqQ j hjL A hA hAj hAsq hAx hAcap s hsL)
...
because `m4_chiSummedShiftBlock_of_freeRow` reads the socket at THIS `A` — the propagation is
...
theorem m4_chiSummedShiftBlock_trivial (R : ChowlaRegime) (M : ℕ) :
  ...
  intro H _ _ L _ q hq hqQ j _ s _ A B hA _ _ _ _ hL4 hfit
  haveI : NeZero q := ⟨hq.ne'⟩
...
⟦the (α) base-cap surgery, JYH-granted 2026-07-30⟧ the base cap `(A : ℝ) ≤ 2·R.x` rides here
because `m4_chiSummedBlockN_of_shiftBlock` reads the shifted family at THIS `A`.  Its
...
theorem m4_chiSummedBlockN_trivial (R : ChowlaRegime) (M : ℕ) :
  ...
  intro H hlo hhi L hLH hnar q hq hqQ A B hA _ _ _ _ hfit
  haveI : NeZero q := ⟨hq.ne'⟩
  have harc0 : (0 : ℝ) ≤ arcDen 12 H := le_trans (by positivity) hqQ
  have hA0R : (0 : ℝ) ≤ (A : ℝ) := Nat.cast_nonneg _
...
landed original with `(K : ℕ)` as a new FIRST binder, the door datum read at the lever
(`doorChiCoeff_gk`, `doorSievedWindow_gk`, `doorSievedCoeff_gk`) and the `Σ_χ` socket's own
row square at `chiFreeRowSq_gk`.  `J` stays `2`; every other byte of the proof text is the
landed one's, with `_gk` names in place of their landed originals.
-/

/-- `chiFreeRowSq` (:154), at the lever. -/
def chiFreeRowSq_gk (K : ℕ) {q : ℕ} (χ : DirichletCharacter ℂ q) (M j X : ℕ) : ℝ :=
  1 / ((X : ℕ) : ℝ)
    * (∫ y in ((X : ℕ) : ℝ)..(2 * ((X : ℕ) : ℝ)),
        ‖((1 / ((2 ^ j : ℕ) : ℝ) : ℝ) : ℂ)
            * shortSum (doorChiCoeff_gk K χ M) (seamS0 (2 * X) ((X : ℕ) : ℝ)) y
                ((2 ^ j : ℕ) : ℝ)‖ ^ 2)

/-- `chiFreeRowSq_nonneg` (:162), at the lever. -/
theorem chiFreeRowSq_nonneg_gk (K : ℕ) {q : ℕ} (χ : DirichletCharacter ℂ q) (M j : ℕ) {X : ℕ}
    (hX : 0 < X) : 0 ≤ chiFreeRowSq_gk K χ M j X := by
  have hX0 : (0 : ℝ) < ((X : ℕ) : ℝ) := by exact_mod_cast hX
  exact meanSq_nonneg (doorChiCoeff_gk K χ M) (seamS0 (2 * X) ((X : ℕ) : ℝ)) ((2 ^ j : ℕ) : ℝ) hX0

/-- `chiFreeRowSq_le_four` (:169), at the lever. -/
theorem chiFreeRowSq_le_four_gk (K : ℕ) {q : ℕ} (χ : DirichletCharacter ℂ q) (M j : ℕ) {X : ℕ}
    (hX : 0 < X) : chiFreeRowSq_gk K χ M j X ≤ 4 :=
  doorRow_trivial_grade_gk K χ M j hX

/-- `M4ChiSummedFreeRow` (:199), at the lever. -/
def M4ChiSummedFreeRow_gk (K : ℕ) (R : ChowlaRegime) (M : ℕ) (RS : ℕ → ℕ → ℝ) : Prop :=
  ∀ H : ℕ, R.Hlo ≤ H → H ≤ R.Hhi → ∀ L : ℕ, L ≤ H → ∀ q : ℕ, 0 < q →
    (q : ℝ) ≤ arcDen 12 H → ∀ j ≤ Nat.log 2 L, ∀ A : ℕ, 0 < A →
      2 ^ j ≤ A → Real.sqrt (H : ℝ) ≤ (A : ℝ) →
      (R.x : ℝ) ≤ 16 * (R.ω : ℝ) * arcDen 12 H * (A : ℝ) → (A : ℝ) ≤ 2 * (R.x : ℝ) →
      ∀ s ≤ L,
        ∑ χ : DirichletCharacter ℂ q, chiFreeRowSq_gk K χ M j (A + s) ≤ RS j H

/-- `m4_chiSummedFreeRow_trivial` (:216), at the lever. -/
theorem m4_chiSummedFreeRow_trivial_gk (K : ℕ) (R : ChowlaRegime) (M : ℕ) :
    M4ChiSummedFreeRow_gk K R M (fun _ H => 4 * arcDen 12 H) := by
  intro H _ _ L _ q hq hqQ j _ A hA _ _ _ _ s _
  haveI : NeZero q := ⟨hq.ne'⟩
  have hAs : 0 < A + s := by omega
  have hterm : ∀ χ ∈ (Finset.univ : Finset (DirichletCharacter ℂ q)),
      chiFreeRowSq_gk K χ M j (A + s) ≤ 4 := fun χ _ => chiFreeRowSq_le_four_gk K χ M j hAs
...
/-- `chiFreeRowSq_le_of_chiSummed` (:248), at the lever. -/
theorem chiFreeRowSq_le_of_chiSummed_gk (K : ℕ) {R : ChowlaRegime} {M : ℕ} {RS : ℕ → ℕ → ℝ}
    (h : M4ChiSummedFreeRow_gk K R M RS) :
    ∀ H : ℕ, R.Hlo ≤ H → H ≤ R.Hhi → ∀ L : ℕ, L ≤ H → ∀ q : ℕ, 0 < q →
      (q : ℝ) ≤ arcDen 12 H → ∀ χ : DirichletCharacter ℂ q, ∀ j ≤ Nat.log 2 L,
        ∀ A : ℕ, 0 < A → 2 ^ j ≤ A → Real.sqrt (H : ℝ) ≤ (A : ℝ) →
        (R.x : ℝ) ≤ 16 * (R.ω : ℝ) * arcDen 12 H * (A : ℝ) → (A : ℝ) ≤ 2 * (R.x : ℝ) →
        ∀ s ≤ L,
          chiFreeRowSq_gk K χ M j (A + s) ≤ RS j H := by
  intro H hlo hhi L hLH q hq hqQ χ j hjL A hA hAj hAsq hAx hAcap s hsL
  have hAs : 0 < A + s := by omega
  have hle : chiFreeRowSq_gk K χ M j (A + s)
      ≤ ∑ χ' : DirichletCharacter ℂ q, chiFreeRowSq_gk K χ' M j (A + s) :=
    Finset.single_le_sum (f := fun χ' => chiFreeRowSq_gk K χ' M j (A + s))
      (fun χ' _ => chiFreeRowSq_nonneg_gk K χ' M j hAs) (Finset.mem_univ χ)
  exact le_trans hle (h H hlo hhi L hLH q hq hqQ j hjL A hA hAj hAsq hAx hAcap s hsL)

/-- `chiFreeShift_pointwise` (:278), at the lever. -/
theorem chiFreeShift_pointwise_gk (K : ℕ) {q : ℕ} (χ : DirichletCharacter ℂ q) (M : ℕ)
    {L j s A B : ℕ} {c : ℝ} (hjL : j ≤ Nat.log 2 L) (hsL : s ≤ L) (hA : 0 < A) (hL4 : 4 ≤ L)
    (hfit : B + L ≤ 2 * A + 4) (hc : chiFreeRowSq_gk K χ M j (A + s) ≤ c) :
    ∑ n ∈ Finset.Ioc (A + s) (B + s),
  ...
  -- ⟦the slack-`4` block bound⟧
  have hslack := sum_Ioc_absWindowSum_sq_div_le_slack4
    (c := doorChiCoeff_gk K χ M) (fun m => norm_doorChiCoeff_le_one_gk K χ M m)
    (seamS0 (2 * (A + s)) (((A + s : ℕ)) : ℝ)) 0 (H := 2 ^ j) (A := A + s) (B := B + s)
    (MS := c) hh0 hAs hfitS hcov hMSrow
  -- ⟦the exchange at the shifted block⟧
  have hterm : ∀ n ∈ Finset.Ioc (A + s) (B + s),
...
/-- `M4ChiSummedFreeShiftBlock` (:397), at the lever. -/
def M4ChiSummedFreeShiftBlock_gk (K : ℕ) (R : ChowlaRegime) (M : ℕ) (F : ℕ → ℕ → ℝ) : Prop :=
  ∀ H : ℕ, R.Hlo ≤ H → H ≤ R.Hhi → ∀ L : ℕ, L ≤ H → ∀ q : ℕ, 0 < q →
    (q : ℝ) ≤ arcDen 12 H → ∀ j ≤ Nat.log 2 L, ∀ s ≤ L,
      ∀ A B : ℕ, 0 < A → 2 ^ j ≤ A → Real.sqrt (H : ℝ) ≤ (A : ℝ) →
      (R.x : ℝ) ≤ 16 * (R.ω : ℝ) * arcDen 12 H * (A : ℝ) → (A : ℝ) ≤ 2 * (R.x : ℝ) →
...
/-- `m4_chiSummedShiftBlock_trivial` (:411), at the lever. -/
theorem m4_chiSummedShiftBlock_trivial_gk (K : ℕ) (R : ChowlaRegime) (M : ℕ) :
    M4ChiSummedFreeShiftBlock_gk K R M (fun _ H => arcDen 12 H) := by
  intro H _ _ L _ q hq hqQ j _ s _ A B hA _ _ _ _ hL4 hfit
  haveI : NeZero q := ⟨hq.ne'⟩
  have harc0 : (0 : ℝ) ≤ arcDen 12 H := le_trans (by positivity) hqQ
  have hP0 : (0 : ℝ) ≤ ((2 ^ j : ℕ) : ℝ) ^ 2 := sq_nonneg _
...
theorem m4_chiSummedShiftBlock_of_freeRow_gk (K : ℕ) {R : ChowlaRegime} {M : ℕ} {RS : ℕ → ℕ → ℝ}
    (hrow : M4ChiSummedFreeRow_gk K R M RS) :
    M4ChiSummedFreeShiftBlock_gk K R M (fun j H => 2 * RS j H) := by
  intro H hlo hhi L hLH q hq hqQ j hjL s hsL A B hA hAj hAsq hAx hAcap hL4 hfit
  haveI : NeZero q := ⟨hq.ne'⟩
  have hAs : 0 < A + s := by omega
  have hP0 : (0 : ℝ) ≤ ((2 ^ j : ℕ) : ℝ) ^ 2 := sq_nonneg _
  ...
  have hper : ∀ χ ∈ (Finset.univ : Finset (DirichletCharacter ℂ q)),
      ∑ n ∈ Finset.Ioc (A + s) (B + s),
          ‖∑ m ∈ doorSievedWindow_gk K M (2 ^ j) n, liouChi χ m‖ ^ 2
        ≤ 2 * chiFreeRowSq_gk K χ M j (A + s) * ((2 ^ j : ℕ) : ℝ) ^ 2 * (A : ℝ)
          + (4 * chiFreeRowSq_gk K χ M j (A + s) + 8) * ((2 ^ j : ℕ) : ℝ) ^ 2 := fun χ _ =>
    chiFreeShift_pointwise_gk K χ M hjL hsL hA hL4 hfit le_rfl
  refine le_trans (Finset.sum_le_sum hper) ?_
  -- ⟦the sum splits into the row sum and the absolute residue⟧
  have hsplit : ∑ χ : DirichletCharacter ℂ q,
      (2 * chiFreeRowSq_gk K χ M j (A + s) * ((2 ^ j : ℕ) : ℝ) ^ 2 * (A : ℝ)
        + (4 * chiFreeRowSq_gk K χ M j (A + s) + 8) * ((2 ^ j : ℕ) : ℝ) ^ 2)
      = (∑ χ : DirichletCharacter ℂ q, chiFreeRowSq_gk K χ M j (A + s))
          * (2 * ((2 ^ j : ℕ) : ℝ) ^ 2 * (A : ℝ) + 4 * ((2 ^ j : ℕ) : ℝ) ^ 2)
        + ((Fintype.card (DirichletCharacter ℂ q) : ℕ) : ℝ)
            * (8 * ((2 ^ j : ℕ) : ℝ) ^ 2) := by
    calc ∑ χ : DirichletCharacter ℂ q,
          (2 * chiFreeRowSq_gk K χ M j (A + s) * ((2 ^ j : ℕ) : ℝ) ^ 2 * (A : ℝ)
            + (4 * chiFreeRowSq_gk K χ M j (A + s) + 8) * ((2 ^ j : ℕ) : ℝ) ^ 2)
        = ∑ χ : DirichletCharacter ℂ q,
            (chiFreeRowSq_gk K χ M j (A + s)
                * (2 * ((2 ^ j : ℕ) : ℝ) ^ 2 * (A : ℝ) + 4 * ((2 ^ j : ℕ) : ℝ) ^ 2)
              + 8 * ((2 ^ j : ℕ) : ℝ) ^ 2) :=
          Finset.sum_congr rfl fun χ _ => by ring
      _ = (∑ χ : DirichletCharacter ℂ q, chiFreeRowSq_gk K χ M j (A + s)
              * (2 * ((2 ^ j : ℕ) : ℝ) ^ 2 * (A : ℝ) + 4 * ((2 ^ j : ℕ) : ℝ) ^ 2))
            + ∑ _χ : DirichletCharacter ℂ q, 8 * ((2 ^ j : ℕ) : ℝ) ^ 2 :=
          Finset.sum_add_distrib
      _ = (∑ χ : DirichletCharacter ℂ q, chiFreeRowSq_gk K χ M j (A + s))
  ...
  rw [hsplit, card_dirichletCharacter_nat q]
  have hrowsum := hrow H hlo hhi L hLH q hq hqQ j hjL A hA hAj hAsq hAx hAcap s hsL
  have hrow0 : (0 : ℝ) ≤ ∑ χ : DirichletCharacter ℂ q, chiFreeRowSq_gk K χ M j (A + s) :=
    Finset.sum_nonneg fun χ _ => chiFreeRowSq_nonneg_gk K χ M j hAs
  have hφq : (q.totient : ℝ) ≤ (q : ℝ) := by exact_mod_cast Nat.totient_le q
  have hφarc : (q.totient : ℝ) ≤ arcDen 12 H := le_trans hφq hqQ
  have hfac0 : (0 : ℝ) ≤ 2 * ((2 ^ j : ℕ) : ℝ) ^ 2 * (A : ℝ) + 4 * ((2 ^ j : ℕ) : ℝ) ^ 2 := by
    positivity
  have h1 : (∑ χ : DirichletCharacter ℂ q, chiFreeRowSq_gk K χ M j (A + s))
        * (2 * ((2 ^ j : ℕ) : ℝ) ^ 2 * (A : ℝ) + 4 * ((2 ^ j : ℕ) : ℝ) ^ 2)
      ≤ RS j H * (2 * ((2 ^ j : ℕ) : ℝ) ^ 2 * (A : ℝ) + 4 * ((2 ^ j : ℕ) : ℝ) ^ 2) :=
    mul_le_mul_of_nonneg_right hrowsum hfac0
  have h2 : (q.totient : ℝ) * (8 * ((2 ^ j : ℕ) : ℝ) ^ 2)
...
def M4ChiSummedBlockMeanSqN_gk (K : ℕ) (R : ChowlaRegime) (M : ℕ) (Bcl : ℕ → ℝ) : Prop :=
  ∀ H : ℕ, R.Hlo ≤ H → H ≤ R.Hhi → ∀ L : ℕ, L ≤ H → (H : ℝ) ≤ arcDen 12 H ^ 3 * (L : ℝ) →
    ∀ q : ℕ, 0 < q → (q : ℝ) ≤ arcDen 12 H →
      ...
      (R.x : ℝ) ≤ 16 * (R.ω : ℝ) * arcDen 12 H * (A : ℝ) → (A : ℝ) ≤ 2 * (R.x : ℝ) →
      B + L ≤ 2 * A + 4 →
        ∑ χ : DirichletCharacter ℂ q, ∑ n ∈ Finset.Ioc A B, (doorChiSup_gk K χ M L n) ^ 2
          ≤ Bcl H * (L : ℝ) ^ 2 * (A : ℝ)

/-- `m4_chiSummedBlockN_trivial` (:546), at the lever. -/
theorem m4_chiSummedBlockN_trivial_gk (K : ℕ) (R : ChowlaRegime) (M : ℕ) :
    M4ChiSummedBlockMeanSqN_gk K R M (fun H => 5 * arcDen 12 H) := by
  intro H hlo hhi L hLH hnar q hq hqQ A B hA _ _ _ _ hfit
  haveI : NeZero q := ⟨hq.ne'⟩
  have harc0 : (0 : ℝ) ≤ arcDen 12 H := le_trans (by positivity) hqQ
  have hA0R : (0 : ℝ) ≤ (A : ℝ) := Nat.cast_nonneg _
  have hL0 : (0 : ℝ) ≤ (L : ℝ) ^ 2 := sq_nonneg _
  have hper : ∀ χ ∈ (Finset.univ : Finset (DirichletCharacter ℂ q)),
      ∑ n ∈ Finset.Ioc A B, (doorChiSup_gk K χ M L n) ^ 2 ≤ 5 * (L : ℝ) ^ 2 * (A : ℝ) := by
    intro χ _
...
-- heartbeats (no tactic search below is unbounded: every arithmetic step is `linarith` or
-- `nlinarith` with explicit hints)
/-- `m4_chiSummedBlockN_of_shiftBlock` (:605), at the lever. -/
theorem m4_chiSummedBlockN_of_shiftBlock_gk (K : ℕ) {R : ChowlaRegime} {M : ℕ} {F : ℕ → ℕ → ℝ}
    {Fan Ftr : ℕ → ℝ} (j₀ : ℕ)
    (hFan0 : ∀ H : ℕ, 0 ≤ Fan H) (hFtr0 : ∀ H : ℕ, 0 ≤ Ftr H)
    (han : ∀ j H : ℕ, j₀ ≤ j → F j H ≤ Fan H)
    (hG1 : ∀ H : ℕ, R.Hlo ≤ H → H ≤ R.Hhi → 2 * arcDen 12 H ^ 7 ≤ Ftr H)
    (hG2 : ∀ H : ℕ, R.Hlo ≤ H → H ≤ R.Hhi →
      108 / 5 * Fan H + 432 / 5 * arcDen 12 H ≤ (4 / 3 : ℝ) ^ j₀)
    (harc8 : ∀ H : ℕ, R.Hlo ≤ H → H ≤ R.Hhi → 8 * arcDen 12 H ^ 3 ≤ (H : ℝ))
    (hfix : M4ChiSummedFreeShiftBlock_gk K R M F) :
    M4ChiSummedBlockMeanSqN_gk K R M (m4BclGraded j₀ Fan Ftr) := by
  classical
  intro H hlo hhi L hLH hnar q hq hqQ A B hA hAL hAsq hAx hAcap hfit
  haveI : NeZero q := ⟨hq.ne'⟩
  have hH0 : 0 < H := by have := R.hHlo_floor; omega
  ...
  · -- ⟦the empty block⟧
    have hzero : ∀ χ : DirichletCharacter ℂ q,
        ∑ n ∈ Finset.Ioc A B, (doorChiSup_gk K χ M L n) ^ 2 = 0 := by
      intro χ
      rw [Finset.Ioc_eq_empty (by omega), Finset.sum_empty]
    rw [Finset.sum_congr rfl (fun χ _ => hzero χ), Finset.sum_const, smul_zero]
    exact mul_nonneg (mul_nonneg hBcl0 (sq_nonneg _)) (Nat.cast_nonneg _)
  ...
  -- ⟦STEP 1⟧ the pointwise maximal bound per character, with the sums already commuted
  have hchi : ∀ χ : DirichletCharacter ℂ q,
      ∑ n ∈ Finset.Ioc A B, (doorChiSup_gk K χ M L n) ^ 2
        ≤ SL * ∑ j ∈ Finset.range (Lg + 1),
            (∑ t ∈ Finset.range (L / 2 ^ (j + 1) + 1),
              ∑ n ∈ Finset.Ioc A B, X χ j t n) * (2 / 3 : ℝ) ^ j := by
    intro χ
    ...
    exact hswap ▸ hstep1
  -- ⟦STEP 1′⟧ the character sum, taken through the assembly
  have hsummed : ∑ χ : DirichletCharacter ℂ q, ∑ n ∈ Finset.Ioc A B, (doorChiSup_gk K χ M L n) ^ 2
      ≤ SL * ∑ j ∈ Finset.range (Lg + 1),
          (∑ t ∈ Finset.range (L / 2 ^ (j + 1) + 1),
            ∑ n ∈ Finset.Ioc A B, Y j t n) * (2 / 3 : ℝ) ^ j := by
    refine le_trans (Finset.sum_le_sum fun χ _ => hchi χ) (le_of_eq ?_)
  ...
  have hshiftY : ∀ j t : ℕ, ∑ n ∈ Finset.Ioc A B, Y j t n
      = ∑ χ : DirichletCharacter ℂ q,
          ∑ n ∈ Finset.Ioc (A + 2 ^ (j + 1) * t) (B + 2 ^ (j + 1) * t),
            ‖∑ m ∈ doorSievedWindow_gk K M (2 ^ j) n, liouChi χ m‖ ^ 2 := by
    intro j t
    rw [hY, Finset.sum_comm]
    refine Finset.sum_congr rfl fun χ _ => ?_
    exact sum_Ioc_shift (fun n => ‖∑ m ∈ doorSievedWindow_gk K M (2 ^ j) n, liouChi χ m‖ ^ 2)
      A B _
  -- ⟦the analytic half: the χ-summed datum, read through the envelope⟧
  have hjtL : ∀ j t : ℕ, j ≤ Lg → j₀ ≤ j → t ≤ L / 2 ^ (j + 1) →
      ∑ n ∈ Finset.Ioc A B, Y j t n
...
/-- `m4_chiSummedN_supplied` (:1036), at the lever. -/
theorem m4_chiSummedN_supplied_gk (K : ℕ) {R : ChowlaRegime} {M : ℕ} {RS : ℕ → ℕ → ℝ}
    {RSan RStr : ℕ → ℝ} (j₀ : ℕ)
    (hRSan0 : ∀ H : ℕ, 0 ≤ RSan H) (hRStr0 : ∀ H : ℕ, 0 ≤ RStr H)
    (han : ∀ j H : ℕ, j₀ ≤ j → RS j H ≤ RSan H)
    (hG1 : ∀ H : ℕ, R.Hlo ≤ H → H ≤ R.Hhi → arcDen 12 H ^ 7 ≤ RStr H)
    ...
    (harc8 : ∀ H : ℕ, R.Hlo ≤ H → H ≤ R.Hhi → 8 * arcDen 12 H ^ 3 ≤ (H : ℝ))
    (hrow : M4ChiSummedFreeRow_gk K R M RS) :
    M4ChiSummedBlockMeanSqN_gk K R M
      (m4BclGraded j₀ (fun H => 2 * RSan H) (fun H => 2 * RStr H)) := by
  refine m4_chiSummedBlockN_of_shiftBlock_gk K (F := fun j H => 2 * RS j H) j₀ ?_ ?_ ?_ ?_ ?_ harc8
    (m4_chiSummedShiftBlock_of_freeRow_gk K hrow)
  · intro H; have := hRSan0 H; linarith
  · intro H; have := hRStr0 H; linarith
  · intro j H hj; have := han j H hj; linarith
  · intro H hlo hhi; have := hG1 H hlo hhi; linarith
