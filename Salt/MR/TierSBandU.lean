...
theorem mrtUniformityXiL2AffW_holds_flat_stride_g12b_band_bU (a h : ℕ) (ha : 0 < a) (hh : 0 < h)
  ...
  obtain ⟨ε, A, hε, hεpin, hεeq, hA162, hA₀A, hbody⟩ :=
    mrtUniformityXiL2AffSet_holds_flat_floor_g12b_band_U a h ha hh hah9 A₀
  ...
  obtain ⟨Kc, hKc0, hKcb, hKcountU⟩ := hcountD
  ...
  have hKcount : ∀ (H : ℕ) [NeZero H], Rd.Hlo ≤ H → H ≤ Rd.Hhi →
      ((bigXiAffD a b h Rd.eps H).card : ℝ) ≤ Kc := by
    intro H _ hlo hhi
    refine le_trans ?_ (hKcountU H hlo hhi)
    exact_mod_cast Finset.card_le_card (bigXiAffD_subset_bigXiAffU a b h hba Rd.eps H)
  intro y hy hlogy
  ...
  have hdoorD : MRTUniformityXiL2Set (fun eps H _ => bigXiAffD a b h eps H)
      (regimeEnlargeX Rd hx') ρ :=
    mrtUniformityXiL2Set_of_subset (fun eps H _ => bigXiAffD a b h eps H) (bigXiAffU a h)
      (fun eps H _ => bigXiAffD_subset_bigXiAffU a b h hba eps H) (regimeEnlargeX Rd hx') ρ
      hdoorRe
  ...
  have htrans := mrtUniformityXiL2AffW_of_set_b9 h (regimeEnlargeX Rd hx') a b ha ha8103
    heps500 hstrideRe hdiv hlo4 hloM hbRe hω8 Kc ρ hKcount hdoorD
