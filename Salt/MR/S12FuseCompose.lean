...
-- The eighteen-slot `hcapWS` body re-elaborates against the wire's own family shape (the same
-- cause as `S11Hoist` §5, from which this block is extracted).
/-- `m4_fuse_hcap_of_capWS (:122)` at the lever. -/
theorem m4_fuse_hcap_of_capWS_gk (K : ℕ) :
    ∃ Cq cs T₀ Kq Ks : ℝ, 0 < Cq ∧ 0 < cs ∧ 3 ≤ T₀ ∧ 0 < Kq ∧ 0 < Ks ∧
      ∀ (R : ChowlaRegime) (M : ℕ) (cU : ℕ → ℂ) (ε : ℕ → ℝ),
        (∀ p : ℕ, ‖cU p‖ ≤ 1) →
        (∀ H L q j A s : ℕ, SocketBase R M H L q j A s →
...
/-- `m4_fuse_hband_of_bandBase (:175)` at the lever. -/
theorem m4_fuse_hband_of_bandBase_gk (K : ℕ) (hMmu : MmuChiRate) (Aexp : ℝ)
    (hAexp : 0 < Aexp) :
    ∃ x₀ : ℕ, ∀ (M : ℕ), 1 ≤ M →
      ∃ C' : ℝ, 0 < C' ∧
        ∀ (R : ChowlaRegime) (C₁ M₀ : ℕ → ℝ),
            ...
            ∀ H L q j A s : ℕ, SocketBase R M H L q j A s →
              ∀ χ : DirichletCharacter ℂ q,
                (∫ t in (-(seamT0 (((A + s : ℕ)) : ℝ)))..(seamT0 (((A + s : ℕ)) : ℝ)),
                  ...
                  ≤ t0BandB (((A + s : ℕ)) : ℝ)
                      (cfbC₁ (((A + s : ℕ)) : ℝ) (C₁ (A + s))) (M₀ (A + s))) :=
  m4_hband_at_door_slot_split_gk K hMmu Aexp hAexp

/-! ## §2 — THE TWO TERMINAL CONJUNCTS -/

/-- `m4_closure_fuse_zero'_nonneg (:225)` at the lever. -/
theorem m4_closure_fuse_zero'_nonneg_gk (K : ℕ) (hK : K ≤ 170000000) :
    ∃ Ct : ℝ, 0 < Ct ∧
      ∀ (Cp : ℝ), 0 ≤ Cp →
  ...
  refine ⟨Ct, hCt, ?_⟩
  intro Cp hCp R M C₁ M₀ ε Kc ρ cU bU t₁ hM hb1 hc1 hbf hgP1 hgRows heps hL4096 hbase hcap
    hband harith
...
-- The statement re-elaborates the full residue against the re-cut prefix (as in `S12Compose`
-- §3/§4).
/-- `logChowla2_capstone_final' (:318)` at the lever. -/
theorem logChowla2_capstone_final'_gk (K : ℕ) (hK : K ≤ 170000000)
    (Aexp : ℝ) (hAexp : 0 < Aexp) :
  ...
  obtain ⟨Ct, hCt, hfuse⟩ := m4_closure_fuse_zero'_nonneg_gk K hK
  obtain ⟨Cq, cs, T₀, Kq, Ks, hCq, hcs, hT₀, hKq, hKs, hcapwire⟩ := m4_fuse_hcap_of_capWS_gk K
  ...
  have hrow : M4ChiSummedFreeRow_gk K R M (m4ChiRowGraded M (fun _ H => RSanDoorRho ρ H)) :=
    hfuse Cp hCp R M C₁ M₀ epsrf Kf ρ liouvilleC
      ...
      (fun _ _ => (0 : ℝ)) hM (fun i m => norm_doorPunctCoeffU_le_one_gk K M i m)
      (fun p => liouvilleC_norm_le_one p) hbf hgP1 hgRows heps hL4096 hbase
      (hcapwire R M liouvilleC epsrf (fun p => liouvilleC_norm_le_one p) hcapWS)
      (hbandslot R C₁ M₀ hbandbase) harith
  -- ⟦THE TWO TERMINAL CONJUNCTS⟧ minted in §2
...
set_option maxHeartbeats 1000000 in
-- Same cause as §4.
/-- `logChowla2_capstone_final_rawcap' (:530)` at the lever. -/
theorem logChowla2_capstone_final_rawcap'_gk (K : ℕ) (hK : K ≤ 170000000)
    (Aexp : ℝ) (hAexp : 0 < Aexp) :
    ∃ (Cg : ℝ) (ε : ℚ) (Kc δ₀ Ct Cq cs T₀ Kq Ks : ℝ) (x₀ Hcap : ℕ),
      1 ≤ Cg ∧ 0 < ε ∧ 0 < Kc ∧ 0 < δ₀ ∧
        0 < Ct ∧ 0 < Cq ∧ 0 < cs ∧ 3 ≤ T₀ ∧ 0 < Kq ∧ 0 < Ks ∧
...
⚠ `DoorBandBase_gk K` remains a HYPOTHESIS everywhere and a theorem nowhere; only its CONSUMPTION
is re-routed, exactly as in §1. -/

/-- `m4_fuse_hband_of_bandBase_graded (:733)` at the lever. -/
theorem m4_fuse_hband_of_bandBase_graded_gk (K : ℕ) (hMmu : MmuChiRate) (Aexp : ℝ)
    (hAexp : 0 < Aexp) :
    ∃ (x₀ : ℕ) (Cb : ℝ), 0 < Cb ∧ ∀ (M : ℕ), 1 ≤ M →
      ∃ C' : ℝ, 0 < C' ∧ C' ≤ Cb * (M : ℝ) ^ (2.1 : ℝ) ∧
        ∀ (R : ChowlaRegime) (C₁ M₀ : ℕ → ℝ),
