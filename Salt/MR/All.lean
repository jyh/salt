...
/-! ⟦STRIDE PAIR RECEIPT⟧ — THE `h`-LANE DOOR RECEIPT WITH THE FLOOR AND THE SCALE EXPOSED, AT A
GENERIC SET FAMILY (`StridePairReceipt`, 2026-09-04, math — λ-BV wave 2-S step F3, MR half; frozen
v1 11:2x, v2 cut on the helm's refuter verdict REPAIR-THEN-FIRE 4/4, fired 12:2x).

`DoorReceipt`'s generator ported to the `h`-lane compose chain (`S16ComposeLH`, `V7RatedH`) with
four deltas: the set is a PARAMETER (`XiFamily`; the road replayed once at `Xi` with the arc
bridge a hypothesis), the scale multiplier is THREADED through the builder
(`chowlaRegimeFlat_exists_param_gen_ceiling_mul`,
`x := a·x₀`; the outer-scale rider cannot carry
`hPHheadroom` scaled by `a`) and carried inert as `StrideScale a R`, the floor is EXPOSED to the
end (H6/H7 at `∀ U1floor ≥ flatDesignBase A` under the one ceiling `loglog U1floor ≤ 3.2A + log
2`), and the door is HANDED OUT at `ρ ≤ 1/(837782·h²)` through the P-slot `MRTDoorReceiptSet`,
which also carries the head's ε-pin as an equality and the count.  The affine arc bridge at cap
`(a·h)·arcDen` on the grid `a ∣ H`, the six forms generated by substitution from the landed hop
statements, the five replays, the door-head, the chain, the receipts (the anti-drift instance
`mrtUniformityXiL2H_holds_flat` is the plain `h`-door on the flat family), and THE CROWN
`mrtUniformityXiL2AffW_holds_flat_stride`: the affine `L²` door AT TAO'S RANGE at a regime sharing
...
#audit_axioms Salt.MR.xceilRider_mul_of_strict
  ...
  Salt.MR.m4_doorL2_supply_Set_gk_khoist
  Salt.MR.m4_second_road_L2_Set_gk_flatRoot_L_khoist
  Salt.MR.flat_roadExit_generic_h
  Salt.MR.flat_capstone_generic_h
  Salt.MR.xceil_arm_split_mul_h
  Salt.MR.flat_conditional_generic_h
  Salt.MR.flat_kswin_generic_h
  Salt.MR.flat_v7_generic_h
  Salt.MR.flat_door_head_xceil_h
  Salt.MR.flat_chain_generic_h
  Salt.MR.mrtUniformityXiL2Set_holds_flat_floor
  Salt.MR.mrtUniformityXiL2H_holds_flat
  Salt.MR.mrtUniformityXiL2AffSet_holds_flat_floor
  Salt.MR.mrtUniformityXiL2AffW_holds_flat_stride
