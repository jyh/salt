...
*(RESTRUCTURED at the venue decision, JYH-ratified 2026-07-15 — see §1¾.
A is now two artifacts: the flagship absorbs the mathematical content +
architecture + condensed ledger; the anatomy message goes to a Notices-style
short piece.)*

**A. The flagship paper** (mathematician-facing; arXiv math.NT →
Forum of Mathematics, Pi).
"A machine-checked proof of Chen's theorem": the theorem and its stack, the
effective intermediates, the condensed ledger + taxonomy, the delimitation
theorems, the method in brief. Skeleton: `papers/flagship/main.tex`.

**A′. The Notices short piece** (broad-mathematician reach; the Gonthier
four-color precedent). "The X-ray of a classical proof": the anatomy
message — where correctness actually lives — in ~8 pages, built on the
catch ledger. Written after the flagship's ledger section stabilizes.
...
## 1¾. Venues (JYH-ratified 2026-07-15: "that's a great strategy")

For the math result (artifact A):

1. **arXiv first** — math.NT primary, cross-list math.LO + cs.LO; the
   tagged repo release is the supplement. GATED on the publication approval
   conversation (the preprint IS publication for policy purposes).
2. **Forum of Mathematics, Pi** as the journal target; the genre precedent
   is Hales et al., *A formal proof of the Kepler conjecture*, Forum Math.
   Pi 5 (2017) — cite it in the cover letter. Risk: a referee discounting
   formalization-of-known-results; the precedent exists to answer that.
3. **Fallbacks in order**: Forum of Mathematics, Sigma (keeps the
   mathematician audience) before Journal of Automated Reasoning
   (guaranteed fit, ITP audience).
4. **In parallel**: the Notices of the AMS short piece (artifact A′) —
   Gonthier, Notices 55 (2008) is the model.
5. **Optional**: a compact self-contained pure-NT note on the genuinely
   new statements ("an explicit bilinear windowed BV", ~15 pp,
   Mathematika/Acta Arith.-tier) — refereeable without Lean; plants the
   classical flag the flagship cites.

**Not**: Annals/Inventiones/JAMS (they publish new mathematics), or any
venue requiring the AI orchestration to be soft-pedaled.

**LaTeX conventions (baked into the skeleton)**: `amsart` master; every
stated theorem carries its Lean declaration name (`\leanname{}`); a
back-matter table maps paper statement → corpus declaration → axiom audit,
regenerated by script at release. Authorship: JYH alone, personal capacity
+ disclaimer; Claude's role in Methods per journal AI policies.

## 2. Audiences and tone

| Audience | Tone | Leads with | Never says |
|---|---|---|---|
| Mathematicians | measured, modest, evidence-first | the mechanism (jointly-uninhabitable packages; §1½ claim 1), then the ledger as its evidence | "referees are obsolete"; any unanchored speed claim |
...
**A. The flagship** *(restructured 2026-07-15 per §1¾; the LIVE section
plan is `papers/flagship/main.tex` — intro / the stack / effective
intermediates / the condensed ledger with the three case studies below /
delimitation theorems / method-in-brief / the artifact / two challenges.
The sketch below predates the restructure; its §§3–4 content moved into
the flagship's ledger section, §§5–6 into delimitation + intro; the
anatomy-first framing now belongs to A′, the Notices piece.)* — sections:
