...
| 2 | sawtooth (7.2)+(7.3)+(7.4) | W5 | **STALE — re-censused 2026-08-08.** (7.2) `sawtooth_fourier_expansion` **[KERNEL]**; (7.4) BOTH arms `norm_majorantCoeff_le` / `…_le_sq` **[KERNEL]**, plus the `L¹` row `tsum_norm_majorantCoeff_le` **[KERNEL]**. ⚠️ **(7.3) ITSELF IS DELIBERATELY NOT ASSEMBLED — it is the CAPTAIN'S CLICK, one click since 16:06.** So this row's residue is *Captain-held*, **not open work**. Was: **[DESIGN]** |
