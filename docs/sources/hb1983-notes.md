...
| **N6** | Weil deltas | p.221 (7.1); p.217 | Estermann `d(k)k^{1/2}(k,u,v)^{1/2}`; the real-primitive `Σχ(ut+u′)χ(vt+v′) ≪ (q,uv′−vu′)` | two *separate* character-sum inputs — the second is stated without proof in HB and must be supplied. ✅ **VERIFIED AT THE SOURCE 8/12** (Captain's order, PDF bytes read at p.217): HB's literal parenthetical is *"(The proof of this is straightforward. We shall omit it.)"* — the claim rides verbatim; the bound as printed is `Σ_{t=1}^{q} χ(ut+u′)χ(vt+v′) ≪ (q, uv′−vu′)` ✅ **PROVED 2026-08-06, AT CONSTANT 1:** `Salt.HB.sum_two_forms_le_gcd_of_isPrimitive` (`Salt/HB/RealPrimStructure.lean`, `1555fe11`; the composite case `62eed044`, `Salt/HB/RealPrimitive.lean`; the prime case `quadraticChar_sum_two_forms_bound_one`, `69ddf6c4`) — re-audited 2026-09-22 at the three axioms with a planted-`sorry` control; **no landed route consumes it** (math's by-name census at `827b59cc7`). *(This cell read "must be supplied" from 08-12 until 09-23; the answer had landed five days BEFORE the Captain's 08-11 question, in a docstring nobody reading this table would meet — the objectives reconstruction of 09-22 surfaced the question and math answered it in seven minutes.)* |
