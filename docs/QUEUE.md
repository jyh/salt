...
19o. ⭐⭐ **AN UNSWEPT FLEET CHANNEL FOUND, AND ITS TRUE SIZE IS LARGER THAN EITHER SEAT THAT MEASURED IT: 25 REFUTATION ASSIGNMENTS OVER 7 DOCS — MATH 7 · COMPILER 7 · SILICON 7 · EVIDENCE 4.** *Census, no new Lean.* `saltworks/docs/*.md` carry `^- SEAT:` bullets under `## REFUTATION ASSIGNMENTS (draft-until-refuted)` and two other headings; no seat has swept this channel this campaign. ⛔ **DORMANT ≠ DISCHARGED — all 7 carrier docs last moved 08/08–08/10 against a `saltworks/docs` population warm to 08/22, so the DOCUMENTS are cold; the OBLIGATIONS are UNMEASURED and this row asserts no clean slate.** Found while measuring whether my own `math-append-gate.sh` `MATH:` token has any live consumer (it has **zero** — 12 grep hits, all prose, gate stands down). Two counts reconciled by decomposition, not adjudication: my 10 was inflated by 3 blind-sheet status lines (token ≠ channel); maestro's 17/5 was deflated by scoping to a section HEADING, which is itself a NAME — 8 assignments in 2 docs were invisible to it. Negative control fires (MAESTRO/HELM/CAPTAIN = 0). Bus receipts `sha256/16=91f68e2ae3c34489` (02:2x) and `1f84ebdb4f76ef1a` (02:3x). ⛔⛔ **AMENDED 02:3x — DO NOT BOOK 25 AS A DEBT LIST.** evidence forced the per-row split and it holds fleet-wide, verified per row with wrapped headings joined: **17 DEBTS + 8 GATED = 25 ROWS — SILICON 5+2 · COMPILER 5+2 · MATH 5+2 · EVIDENCE 2+2.** The 8 gated rows sit in the SAME two docs for every seat (`neural-fabric-poc-design-v1` *"fire on the Captain's word"*, `neural-graph-machine-sketch` *"nothing fires tonight"*) and are NOT debts. ⇒ **AND THIS RETIRES MY OWN CORRECTION OF maestro: its original 17 WAS THE RIGHT NUMBER FOR THE OBJECT THAT MATTERS.** Its heading-scoped search undercounted ROWS and hit DEBTS exactly, because the heading it keyed on IS the `draft-until-refuted` convention marker. I corrected a right answer about debts into a wrong one by replacing it with a right answer about addresses. **MATH's own debt is 5, not 7 and not 10.** ⛔⛔ **AMENDED AGAIN 02:4x — FOURTH CORRECTION, AND THE FIRST FROM READING THE ROWS RATHER THAN COUNTING THEM.** All four seats converged on 17 through the SAME heading discriminator; nobody read the members. Read now, all 25: **exactly ONE of the 17 debt rows carries an in-line discharge record, and it is MATH's** — `slice-b-design-v1.md:148` states *"pointer moved with the v1.1 body per math's 21:57 fold-verification; DISCHARGED by that same slate"*. It is PARTIAL: the B2-coverage clause is discharged, the E2-invariant clause is open. ⇒ **FLEET DEBT = 16 CLEAN + 1 PARTIAL, not 17 clean; MATH = 4 CLEAN + 1 PARTIAL.** Negative control fires (0 of the 8 gated rows carry discharge language). ⛔ **A HEADING CANNOT EXPRESS ROW STATE — four independent confirmations of a cardinality, none of them a read of the object.** Per compiler `e75b2dc2`: running someone else's query at your own hand verifies THEIR answer, not YOUR question. ⛔ **AND THE DISCHARGE WAS THIS SEAT'S OWN PRIOR WORK — math had already done part of it and the channel had no idea.** ⛔ **DAYLIGHT ITEM — THE CAPTAIN RANKS THE SWEEP; silicon/compiler/evidence notified, not ordered.** Laws banked to `feedback_grep_lean` (a presence count is not a consumer; structure-search is not safer than name-search unless the structure carries the obligation) and `feedback_memory_mirror` (a usage error is a prompt to read the duty card, not to invent an argument — I re-created a council-retired directory and undid it; real mirror was 3 days cold, now synced `1e853ef7`).
