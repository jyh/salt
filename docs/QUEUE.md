...
✅ **K6 and K8 RAN — math, 2026-09-17 04:29, BOTH CONFIRM** (report seat `ca763d977`; the S-3 statement holds under a second non-author arm). **K4 and the full K9 drive remain UNRUN** (author-driven, labelled weaker) and are not claimed. math's note for the cap's next re-derivation: `log 8103 = 8.99998964…` clears `≤ 9` by 1.04·10⁻⁵ and `log 8104` FAILS — the lattice top has no room. *(This line read "K6 and K8 are UNRUN … routed to math as unblocked work at its tier" until the helm edited it 09-17 05:2x on kent's digest #4 item 4; QUEUE.md is maestro-owned, so the row edit was the helm's.)*
✅ **S-3 — LANDED 2026-09-14 19:3x (h2c, life 64, on `jyaletheia`, as LAUNCHER; ONE Opus 5 executor, 16 minutes wall clock, 19:15 → 19:31; the helm's FIRE word 19:04:44 on freeze v1.1 `d227b5700`, math's non-author SURVIVES `6cf7f7e60`). THE CHARTER ROW ABOVE IS DISCHARGED AS TO S-3 AND KEPT AS WRITTEN.** `Salt/MR/TierSBandU.lean`: **8 declarations = 1 `def` + 7 `theorem`, all 8 audited in `All.lean` at `[propext, Classical.choice, Quot.sound]`**; `saltbuild EXIT=0` on `Salt.MR.All` (9397 jobs, 0 sorry warnings, 0 warnings attributable to the file); the freeze's byte-guard `check_s3_statements.py 74afa591` reads `STATEMENTS IDENTICAL` 8/8 at every commit (U3's docstring differs by the one permitted parenthetical, the `:520-522` → `:524` pin). Six obligations, six landed, **zero flags, max 2 attempts on any name** (U3's first attempt: `Finset.le_sup` with `f` left to unification picked `Subtype.val`; the second pins it). The statement is the crown band's 159-line body transcribed by script with exactly six edits; U5 is one `exact`; the wave's real work was U2's numeral (`8103·32·2^70·500^10·3^40·8103^15 ≤ 2^539`, 103 bits spare, `norm_num`) and U3's `Finset.sup` threshold.
⛔ **NO NEW UNCONDITIONAL THEOREM, AND NOTHING BEARS ON TWIN PRIMES** — the honest label is the freeze's own first line and it does not move on landing: the load-bearing export is the BAND `(x₀, ω₀, Hhi₀)` alone (`ε` is a theorem of the landed statement; a per-class `A` is a finite `max` at D10); at fixed `z` Tier S is a SECOND PROOF of a terminal that already lands; `E2` stays conditional on `MRTDoorAllGrades`, which has no producer; the `∃ε` tripwire is untouched. **What this does not buy: §8 (the entropy-side `∀ Ra` arrow D12's `hwin` needs at every band regime) is still UNSTATED** — routed above as the next rung, statement first, non-author refuter. K6 and K8 remain the non-author second arm math holds in parallel; a KILL there is statement-level and would reopen this row, not close it.
✅ **S-4 (THE ENTROPY-ARROW BRIDGE) — LANDED 2026-09-14 21:0x (h2c, life 66, on `jyaletheia`, as LAUNCHER; ONE Opus 5 executor, 14 minutes wall clock, 20:47 → 21:01; the helm's FIRE word 20:43:05 on freeze v1.1 — salt `8f0b1443`, rebased onto #139 as `b659642f`; seat `aa9858f91` — after math's non-author pass NO KILL `853b51201`). §8's ROW ABOVE IS DISCHARGED AS TO THE BRIDGE AND KEPT AS WRITTEN.** Two new files, no landed file touched: `Salt/Entropy/Chowla/StrideShellBand.lean` (M `flatDesignBase_mono` · H `log_chowla_aff_of_door_at_regime_g12b` · C `affGrade_composes_g12b` · K1 `gradedAffHeadAt_g12b_of_at_regime`) and `Salt/MR/TierSBridge.lean` (K1′ · B `band_not_logChowlaFailsAff_g12b` · K2 `affFullRangeAt_band_of_not_fails` · K2′ `band_Hhi_unbounded_g12b`); **all 8 audited in both `All.lean` at `[propext, Classical.choice, Quot.sound]`**; `saltbuild EXIT=0` on `Salt.Entropy.All` (8844 jobs) and on `Salt.MR.All` (9411 jobs), 0 warnings attributable to either file; the byte guard `check_s4_statements.py` reads `STATEMENTS IDENTICAL` 8/8 at every commit — no declaration docstring moved. Two obligations, two landed, **zero flags, ONE attempt on each**: H is the landed head's 181 lines with the crown call deleted, and NO step needed a fact about `Ra` beyond the four hypotheses and the structure fields — the freeze's §3 promise held; C is `StridePrize.lean:332-394` with `δ₀ := floor`, `hδ₀ge := le_rfl`. The one repair between freeze and wave was the REFUTER's, decided in the kernel with a firing negative control: B's `A₀` export was INERT (B at `A₀ := 0` re-packed to B at every `A₀`; the identical re-pack REFUSED on S-3) — v1.1 added the band conjunct `flatDesignBase A ≤ Hhi₀`, B's proof from H and C on the refuter's route, and K2′ as the receipt that along `A₀ → ∞` the band's `Hhi₀` is unbounded. Executor's own finding, recorded: a `#print axioms` audit through `lake env lean` reads the OLEANS on disk, so run right after C landed it still printed `sorryAx` for B and K2′ until `Salt.MR.TierSBridge` was rebuilt — an axiom audit is a claim about the oleans, not the source.
⛔ **NO NEW UNCONDITIONAL THEOREM, AND NOTHING BEARS ON TWIN PRIMES — AND B AND K2′ CARRY THE REFUTER'S OWN CONSTRUCTION** (math's P6 and P7, transcribed by the author): they are unconditional now WITHOUT a non-author pass of their own — the statement was refuted by a non-author, the repair was not; a third-party look at B/K2′ is a ROW, not a gate (the helm 20:43). What this buys: for every admissible class `b < a` and every `y` in ONE band `[x₀, exp(31·Hhi₀/ε)/a]`, log-Chowla does not fail (B); read into D12 and out as `AffFullRangeAt a b h ε (ε·log ω₀ + log x₀ + 1) N` at every `N` under the roof (K2); the roof unbounded along `A₀` (K2′). **What it does NOT buy: D10's ONE `(ε, A)` over an INFINITE `S`** — the `(2ε, 1)`-normalisation inside a band (`ε·log ω₀ + log x₀ ≤ ε·log M` in the band's upper part, reading only the exported ceilings) and the class-scale placement `(N − r)/P ∈ [x₀, roof]` are the NEXT LINK, arithmetic on exported terms as read at the freeze, unstated and unpriced here. The bridge inherits the composition's `a ≤ 2310` (at `a = primorial z` that is `z ≤ 12`; `z = 13` fails the door arm by 13× — a GRADE question nobody owns). THE NAME: this rung is THE ENTROPY-ARROW BRIDGE and it KEEPS `S-4`; the Tier S block's plateau item was RENAMED **S-PLATEAU (THE PLATEAU)** by the helm 2026-09-14 22:0x (desk `NI`, after #139's pointer), so the two no longer share a number — and every citation written before tonight that says "S-4" may mean either, so cite by the WORDS.
✅ **THE LADDER — LANDED 2026-09-14 23:1x (h2c, life 68, on `jyaletheia`, as LAUNCHER; ONE Opus 5 executor, 26 minutes wall clock, 22:50 → 23:16; the helm's WAVE WORD 22:28:00 with THREE conditions — the guard repair first, P6 in the wave, the pool re-read at the fire moment (`jyaletheia` 53/82/72) — on freeze v1.1 salt `bba88020`, cut ON TOP OF the refuter's object `54efae25`, which stays reachable; seat `e2bd75d66`; after math's non-author pass NO KILL, report seat `687739ad7`, Pl proved as a ∀ in exact rationals for every `a ≤ 2310` and every `Hhi₀ ≥ 2^600`). Commits `38aec00a` (Pl) · `9807ba0f` (L) · `eb2c11d1` (the aggregate); **7/7 at `[propext, Classical.choice, Quot.sound]`** (Sc at `[propext]`); the three-arm guard (bytes · skeleton · kernel types) IDENTICAL at every commit and re-run by the launcher at the landing; `Salt.MR.All` audit ✓ on all seven. THE SCALE QUANTIFIER MOVED — the bridge moved the REGIME quantifier, S-3 the CLASS quantifier — AND D10 FIRED IN THE KERNEL FOR THE FIRST TIME.**
⛔ **NO NEW UNCONDITIONAL THEOREM, AND NOTHING BEARS ON TWIN PRIMES — AND hah9 AND P6 (THE CROWN TWIN) CARRY THE REFUTER'S OWN CONSTRUCTION** (math's probes on freeze v1, transcribed by the author at v1.1): K's set `{n | twinLogWeight P n ≠ 0}.Infinite` at squarefree `P ≤ 2310` is the sieve-level set the crown road already lands, and `twinLogWeight_support_infinite_of_crown_g12b` states it from the crown road's landed names at every `0 < P ≤ 2310` — so K is a SECOND statement of the terminal on `main`, not the first (math's M11(a): without the twin the honest label would have drifted upward on its own, from "a second proof" to "the first statement of the terminal"). The two roads meet at ONE theorem. `P ≤ 2310` is the bridge's cap (the composition's `a ≤ 2310`), inherited and not added. The byte guard's hole math found (M9: a `variable`+`include` above the block gives every TYPE a binder while the block guard reads IDENTICAL) is closed on three arms and driven against that mutant. Desk NN — the refuter-supplies/drafter-transcribes shape, four names over two rungs — is the Captain's and holds nothing here. THE LADDER TAKES NO NUMBER (`S-5` refused on evidence: a July council item and a July lemma already carry it); cite it by its words. ⚠️ **A LEAN LAW FROM THIS WAVE (the executor's, met from the other side by math's C1 control): `norm_num` DECLINES `2 ^ 600` with a warning and leaves the goal open (`exponentiation.threshold` is 256), and a `2 ^ 600 ≤ Hhi₀` hypothesis left in the context made THREE unrelated later tactics — `positivity`, `nlinarith`, `norm_cast` — die on `maximum recursion depth`: one cause, three errors, none naming it. ⇒ Extract the small floor you need (`2^64 ≤ Hhi₀` under a local `set_option exponentiation.threshold 700 in norm_num`) and `clear` the big numeral.** (The `salt/CLAUDE.md` build-section copy is the helm's, by its 23:19 offer.)

✅ **ROAD F · THE DOOR AT THE HEAD'S GRADE, RUNG 2 — LANDED 2026-09-16 (h2c; PR #149 → merge `e93cc515` on `main`).** `Salt/MR/FlatDoorEpsRung2.lean`, **5,755 lines, `sorry` 0 at the merge**, `flatDoorEpsFamilyW_holds` proved; imported from `Salt.MR.All` (3 sites). ⛔ **THIS LINE IS ITSELF THE FIX FOR A DEFECT: the landing sat on `main` for a day and the QUEUE carried it NOWHERE** — measured at `origin/main` `851d397c`, `FlatDoorEps` · `flatDoorAllGradesW` · `rung 2` each **0** against a live control (`crown` 32). A rung that lands and is not routed is a rung the next seat re-derives.

⚖️ **ROAD F · THE CROWN'S STATEMENT-ONLY FREEZE — THE CAPTAIN RULED THE STATEMENT, 2026-09-17 AFTERNOON COUNCIL ⑮, HIS WORD: "A".** Commissioned that morning (⑩: the freeze to h2c, **no fire**). h2c's statement-only freeze (seat `789083467`) · math's non-author pass (seat `299d046ef`) · v1.1 as a DELTA on v1, which is not edited (seat `94b3937f1`); §G stamped **A** at h2c's life 82 (seat `22d9efa86`). *(The freeze and the pass are seat-record artifacts and are cited by seat sha; the private record's paths do not enter this repo.)*
  - **ARM A, RULED:** the statement stays EXACTLY as math confirmed it — **69 tokens, v1 = v1.1, diffed BY SCRIPT with a one-numeral control that differs** — and **non-vacuity lands as SEPARATE theorems**. Arm B (the conjunct inside the statement) is CLOSED; no v1.2 is owed. B's content arrives anyway as a theorem the moment W-δ lands.
  - **math's VERDICT: CONFIRM, NO KILL**, 16 rows each marked DROVE or READ. Zero level 1 is the landed door **token for token, 65 tokens, re-derived by math with its own mutation control** — it did not accept h2c's diff as evidence of h2c's own claim. Its FINDING is accepted: §7 treated **K6b and K7 as independent rows and they are one fact**, so h2c WITHDREW v1 §7's severity sentence; **K6a** (W-δ true at its degenerate values) stands CONFIRMED and is the statement-killing half.
  - ⛔ **NOTHING HAS FIRED AND NOTHING IS LANDED — salt is untouched, as ⑩ required.** The owed control is measured in SCRATCH ONLY and yields `crownNV_landedW_nonvacuous` (unconditional, about **landed** W), with the receipt in the PAIR form ruled on `main` this morning (`4474accb`): `saltbuild EXIT=0` · 0 errors by the ruled needle · the axioms line · a planted `sorry` firing on its own name only · a one-numeral mutant → `EXIT=1`.
  - ⚠️ **NV-c IS PRICED, NOT DRIVEN** (v1.1 ADDENDUM 1): that the `ξ = 0` term is positive is the parity floor at an odd `H` — **class B, ~120–150 lines, six steps**, the risk being integrability bookkeeping rather than mathematics; one needle for a landed monotonicity lemma returned 0 and was **not re-driven by a second method**. The floor constant stays NON-EFFECTIVE. **W-δ's truth depends on none of it.**
  - ⚠️ **THE INDEPENDENCE IS DECLARED-PARTIAL AND THE HOLE IS THE HELM'S, NOT math's:** K7's substance reached math because the helm's relight post quoted h2c's K7 nomination in full. math excised §7, wrote M1–M13 from the remaining 109 lines, and said so in its own §METHOD — *"the coupling M5 reports is mine; the premise is not."* The commit order proves criteria-before-VERDICT (08:31:16 → 08:52:24); **it cannot prove criteria-before-READING, because reading leaves no commit.**
  - **NEXT, and it is h2c's:** the landed-W non-vacuity module (~110 lines, a transcription, **needs no further word**), then the separate non-vacuity theorems under arm A. ⛔ Sequenced by the budget, not by this line: h2c is on `saltforge`, which read **76 against a Thursday all-models line of 75** at the ruling.

✅ **ROAD F · W-δ — LANDED 2026-09-18 (h2c; PR #156 → merge `8dd67415` on `main`), AND THE SECTION ABOVE SAYS "NOTHING IS LANDED".** That sentence was TRUE when it was written (09-17) and is SUPERSEDED HERE; it is left in place as its own dated record. ⛔ **THIS ROW IS THE SAME FIX AS THE RUNG-2 ROW ABOVE, FOR THE SAME DEFECT, A SECOND TIME:** measured at `origin/main` `046acf6b` on 09-19, `FlatDoorAllGrades` · `flatDoorAllGradesW_holds` · `FlatDoorNonVacuity` · `#156` · `#157` · `8dd67415` each scored **0** in this file against live controls (`rung 2` 1 · `crownNV_landedW_nonvacuous` 1) — the landing sat on `main` for ~22 hours routed nowhere.
  - **WHAT LANDED (#156):** `Salt/MR/FlatDoorNonVacuity.lean` (162 lines) and `Salt/MR/FlatDoorAllGrades.lean` (178 lines), 12 names at the three axioms. `flatDoorAllGradesW_holds : FlatDoorAllGradesW` — the `L²` MRT door on the flat family at EVERY grade `ρ > 0` and every `ε ∈ (0, 1/500]`, the statement the Captain ruled (arm A, token for token). Non-vacuity as SEPARATE theorems: `crownNV_landedW_nonvacuous` (landed W) and `crownNV_Wdelta_nonvacuous_holds` (W-δ) — the regimes can be taken with `0 ∈ Ξ_H` at every `H ≥ Hlo`. math's non-author pass on the landed module: **CONFIRM, NO KILL**, its own build + axioms PAIR from its own tree.
  - **THE FOUR DATED DOCSTRING ERRATA LANDED (#157 → merge `046acf6b`):** the four landed sites that said the flat door has ONE grade, or that W-δ is unstated, each keep the stale sentence in place with an end-of-file dated erratum; 71 insertions, no statement moved. One of the four is a frozen file, edited docstring-only under a recorded one-time exception.
  - ✅ **NV-c (THE PARITY FLOOR) — LANDED WITH PR #158, 2026-09-19 (h2c).** *(This bullet read "DRIVEN AND UP AS A DRAFT, NOT LANDED" from #159's merge until #158's; it is re-stamped in the commit that lands it, so the landing and its routing are ONE merge.)* `Salt/MR/FlatDoorParityFloor.lean`, 222 lines, 12 theorems at the three axioms: the `ξ = 0` term of the door is `≥ 1/H²` at odd `H`; EVERY `ChowlaRegime` has `Hlo + 1 ≤ Hhi`; a regime with `0 ∈ Ξ_H` on its range carries the door only at `ρ ≥ 1/(Hlo+1)²`; on the flat family above the non-vacuity floors `∃ R ∀ ρ` is FALSE (`crownK6_flat_order_forced` — K6b, in the honest form v1.1 named). The price was 6 declarations with an integrability risk; the risk did not exist (a landed one-liner) and the monotonicity lemma whose needle returned 0 on 09-17 IS landed (`chowlaTower_monotone`). **math's NON-AUTHOR PASS: CONFIRM, NO KILL** (seat `03a45ce93`; criteria committed before the object was opened): its own build + axioms PAIR from its own tree; the binders token-identical to `crownNV_above_flat_floor` (44/44); both halves of "the order is forced" driven on ONE class with ONE `H₀`; three mutants red at the predicted lines, two of them with proved witnesses. ⛔ **LIMITS, BESIDE THE VERDICT:** the refutation over ALL regimes is NOT claimed — the Mertens floor is non-effective, and the control `crownK6_Wdelta_regime_moves` shows no `H₀` in its statement while its proof still sits above one (math's §3: *an `∃` can hide a floor*); nothing at even `H`; and the argument formalised is math's own 09-18 sketch, so math is a non-author of the Lean and NOT an independent judge of whether it was the right thing to prove.
  - ⚠️ **WHAT IS STILL NOT SAID, UNCHANGED BY ANY OF THIS:** the shape is `∀ A₀ ∃ R` on the FLAT family, not the crown's `∃ H₀ ∀ R`; **the crown still has no producer** and E2's `hcrown` binder STANDS. No rate is exported. Nothing here bears on twin primes.
  - **NEXT: road F has NO commissioned hop.** The next hop toward the crown is a STATEMENT, and a statement is the Captain's word at a sitting; h2c holds on a gate until one is ruled.

✅ **ROAD F · ARM Z — THE STRIDE DOOR AT ALL GRADES — LANDED 2026-09-23 (h2c; PR #165 → merge `239bb106` on `main`), AND THE LINE ABOVE SAYS "road F has NO commissioned hop".** That sentence was TRUE when it was written (09-19) and is SUPERSEDED HERE; it is left in place as its own dated record, the same form as the two rows above it. The hop was commissioned by the Captain's word at the 2026-09-22 sitting ("yes (a)": take Z; "(a)": the N7 fence lifted for Z ONLY), priced at ≈2,600–2,950, continued on his "continue" at ≈4,650, and landed at 5,908 lines (≈1.27×), the movement reported in the helm's ledger at each step and never re-put to him as a question.
  - **WHAT LANDED (#165, 18 commits + the merge):** `Salt/MR/StrideDoorAllGrades.lean` — **`strideDoorAllGradesW_holds : StrideDoorAllGradesW`** at the three axioms, 121 names (112 theorems, 9 defs) registered under `#audit_axioms` in `Salt.MR.All` with a ⟦TIER S — ROAD F, THE STRIDE AXIS⟧ block; the statement of record byte-identical to the freeze's on every commit (guard arm B). Zero level A re-packs the LANDED crown's conclusion verbatim and so carries the cap in code by necessity: it sits under ONE named guard window (`strideDoor_zero_level_g12b` through its `example`), the exemption driven red-first, re-cut once on math's finding and re-driven.
  - **NON-AUTHOR READS (math, Road F's refuter, criteria and mutant predictions committed BEFORE each diff was opened):** halves 1–2b CONFIRM-WITH-LIMITS, the delta CONFIRM-WITH-LIMITS, both NO KILL; every limit answered on top in `3fa4cbaf` (zero level A registered; three HELD surfaces corrected; the guard window re-cut). The first CI build that elaborates the file is the one at `8635b82b`; a green badge before the module was rooted was not a verdict, and was not read as one.
  - **THE BUILD IS THE CENSUS (freeze ADDENDUM 4):** half 2b's executor stopped itself on the brief's own stop condition when three cap sites appeared that no reading census could see (a callee's conclusion carries the capped variable into its callers); the remedy is a mechanical transcription of every owed body built ONCE, whose failing set IS the population of cap sites. It worked: the v2 build failed at exactly the eight named rows.
...
the door.**
⚠️ **MEASUREMENT STAMP 2026-08-23 02:5x (math) — THE RATIFIED OBJECT LINE ABOVE IS NOT REWRITTEN; THIS IS A STAMP.** The clause's *"⟦THE CLASS PRICING⟧ open — a design question"* **names the wrong open thing, while being right that something is open.** Measured: (i) ⟦THE CLASS PRICING⟧ **as a design question is ANSWERED IN THE KERNEL** — `Salt/MR/M4ClassPrice.lean` (`⟦PART C⟧ — THE CLASS PRICING`) is 1,114 lines, **41 theorems, ZERO `sorry`** ⚠️ *(ERRATUM 2026-08-25, D-1 pricing wave: this stamp said **42**. It is **41**, and the 42 was NEVER right — it is a GREP ARTIFACT. `grep -cE '^(theorem|lemma)'` returns 42 because **line 553 is PROSE inside a docstring** beginning at column 0 with `lemma's \`hslack\`, at ...`. `grep -cE '^theorem '` returns 41. The file has not changed since `1da25df5` (2026-07-31), so this was a miscount on 08/23, not drift. Reproduced and verified at the bytes by the wave AND independently at the helm.)*, **39 of its 52 declared names registered** in `Salt/MR/All.lean` ⚠️ *(ERRATUM same pass: the **11** was a count of the STRING `M4ClassPrice` in All.lean — 10 prose lines + 1 import — not name registrations. The 13 unregistered are 5 arithmetic helpers and the 8 `_gk` twins. ⛔ And registration licenses **≤ the 3-axiom whitelist**, NOT `= [3 axioms]`: no doc states an axiom count for any name here.)*, 3 importers, and its own header states it *"lands the second, ⟦THE CLASS PRICING⟧, in the shape the residue design v2 froze — at depth 1, with no induction."* (ii) **BUT THE RESIDUE PREDICATE IS UNPRODUCED IN BOTH LANES.** Full-tree census of `M4RowMeanSq_L`: **11 occurrences — 2 defs (`:991`, `_gk :1029`), 2 binders (`:1392`, `:1422`), 1 arrow-antecedent (`:3434`), 6 prose — and NO PRODUCER**, confirming this seat's own earlier row at `MRTPropA3.lean:890`. Same census on the **unprimed** `M4RowMeanSq` / `M4RowMeanSqUnphased`: **also no producer** (1 binder-site, 0 conclusions / 0, 0). ⛔ **SO THERE IS NO LANDED TEMPLATE TO PORT ACROSS — the L lane is not "the unprimed lane's work, re-indexed."** ⛔ `M4ClassPrice` mentions `M4RowMeanSq_L` **ZERO times**; it serves the unprimed residue, and the L lane reaches its machinery only through the socket exit `m4_sievedDoorSq_of_classMeanSq_L` (`M4RowLinear.lean:3186`). ⇒ **THE PRECISE OPEN OBJECT: a producer for `M4RowMeanSq_L` — a per-block mean-square bound on the PHASED sieved short sum over the door ladder** (`M4RowLinear.lean:991`), which its own docstring says should descend from `m4_meansq_per_chi_gen_L`. ⚠️ **NOT CLAIMED: that the descent is routine** — the docstring names two obstructions and this stamp did not price them. ⛔ **AND THE DISCIPLINE NOTE, because it nearly went the other way twice:** this seat twice built toward *"the queue clause is STALE"* and the evidence refused it both times — first the primed/unprimed object test (`M4ClassPrice` serves the unprimed lane), then the producer census (the unprimed lane is unproduced too). **The staleness reading was the one that would have made this seat more right, and it was wrong both times.** The clause stands; only its label is refined. ⇒ Door road unchanged elsewhere: `mrtUniformityXi_of_absWindowBound_twelve` (`M4Window.lean:268`) still concludes the door from ONE `L¹` estimate, arc side unconditional.
⚠️ **MEASUREMENT STAMP 2026-09-02 18:2x (math, Fable; kernel walk at `980c5c4f`, refuter-passed 3/3, the EM freeze v2) — THE OBJECT LINE IS STALE IN FOUR PLACES AND IS NOT REWRITTEN.** (i) the residue `M4RowMeanSq_L` is the PLAIN route's socket — two consumers, both on that route (`m4_blockMeanSq_of_rowMeanSq_L`, and the route's own terminal `m4_wave_exit_L`, which has ZERO dependents), zero producers, OUT of the class road's cone (`m4_wave_collapsed_L`, "ANALYTIC ARMS — none"), abandoned by design (`M4ClassPrice` ⟦U2⟧); RETIRED as an object, bytes untouched. (ii) ⟦THE CLASS PRICING⟧ is LANDED in the `_L` lane too (`m4_classMeanSq_of_chiMeanSq_L` · `m4_nonCoprime_classMeanSq_N_L` · `m4_classBlockMeanSq_of_rowDatum_L`, 08/01) — behind data-side envelope gates (`han`/`hG1`/`hG2`) whose `H`-uniform inhabitation at `Hhi` is unmeasured (the register's anti-vacuity duty). (iii) the L¹ door has 27 direct / 125 transitive dependents, **0** of them in `logChowla2_v7_rated`'s cone — the live lane walks the L² door `MRTUniformityXiL2`. (iv) the L² door was PROVED inside V7 (`ρ ≤ δ₀`, `ε = 1/500`, `R.Hlo = flatDesignBase A`) and NEVER STATED; **it is NAMED since this stamp: `Salt/MR/DoorReceipt.lean` — `mrtUniformityXiL2_holds_flat` (the L² door at grade `≤ 1/837782` on the flat family, UNCONDITIONAL) and `mrtUniformityXi_holds_flat` (the Tao-faithful L¹ door at `√(1/837782)`, by the bridge `mrtUniformityXi_of_xiL2`)**, each replay tied to its landed hop by kernel receipts (`logChowla2_v7_rated_form`, `logChowla2_v7_rated_of_generic`). The crown object is **`MRTDoorAllGrades`** (the door at EVERY grade, Prop 2.4 for `λ`; a `def`, NO producer — the E-ladder's consumer, so this row and P2 item 9 can re-anchor on a kernel object; that re-anchoring is the helm's). ⇒ A RESIDUE IS PRICED AT THE ROAD THE LIVE HEADLINE WALKS, NOT AT THE ROAD THE ROW WAS WRITTEN ON. Freeze: `2026-09-02-math-EM-door-FREEZE.md` (§11 = v2). **Nothing here bears on twin primes.**
...
```
  MRTUniformityXi
    ← mrtUniformityXi_of_absWindowBound_twelve  (M4Window:268)   — concludes the door from ONE L¹ estimate
       ...
       ⟦CLASS PRICING⟧ M4ClassPrice.lean                          ✅ 41 thms, 0 sorry — design question ANSWERED  [42→41 erratum 08/25, see :196]
...
19p. ✅⭐⭐⭐ **A.4(ii) WHOLE AT θ = 3/4 IS IN THE KERNEL, UNCONDITIONAL, IN THE SOURCE'S QUANTIFIER ORDER — AU D★ EXECUTED 2026-09-02 (math, Fable head; helm-fired 09:07 after the refuter pass).** `Salt/MR/A4FLargeRange.lean` (NEW, 7 names, every theorem `[3 axioms]`, audit-listed in `Salt/MR/All.lean`, FIRST ATTEMPT each): `mrt_large_range_parametric` (the port of the mid-range theorem to `(log X)^{20} < |u| ≤ 2X` at the floor `(log X)^{3/4}(loglog X)^5 ≤ log Y`, uniform `C`) · `MRTLargeRangeEquidistributionFixedEps` (D1, `∀ε ∃C`) + `_holds` · `MRTLemmaA4iiFixed34E` (D2, `∀ε` before `∃C₀`) + **`mrtLemmaA4iiFixed34E_holds` — the threshold producer with no hypothesis left: high-M + mid + large** · the two pins (frozen ⇒ new). ⭐ **THE FINDING: D★ WAS PRICED CLASS D AT A FLOOR NO CONSUMER READS** — the frozen Prop's one consumer instantiates it at `Y = exp((log X)^{3/4+ε'})`, where the landed VK mechanism has no height cap; at the object the block is ONE class-C port with one new rpow stone. ⛔ **NOT discharged, on purpose:** `MRTLargeRangeEquidistributionFixed` and the T-form's large arm demand a uniform `C` at a BARE exponent — STATEMENT-BLOCKED by every ζ'/ζ route (needs `(loglog)^a` above the exponent), not refuted; the source never claims it. Flags entry 09-02 (the S2(iii) shape). Statement-tier cost recorded: `3/4 + ε` vs the paper's `2/3 + ε` (`0.045423` vs `0.060563`), forced by the landed region. Demand-side item for the helm, ripens with A.5: the ladder above inherits `≪_ε`. **Nothing bears on twin primes.**
...
6. ✅⚖️⭐ **COMMISSIONED AS *WORKER-TIER* — HELM RULING, 2026-08-26 06:0x. IT IS NO LONGER A DESIGN
   ...
   **(b)** the rethread, as a SIBLING chain, four commits ending salt `6b69a783`:
   ...
   ROW'S:** R-1 Wave 1, authority the helm's R-1 Wave 1 commission brief (2026-08-30, in the
   private record; helm-ruled, released by 2(b)'s receipt). `Salt/MR/V7Rated.lean`: the rated supply sibling
...
7b. ✅⚖️ **RULED AND COMMISSIONED 2026-08-26 11:1x (helm, Fable) — NO LONGER DESIGN-FIRST; MATH
   ...
   **the same two data the `h = 1` door closes onto**: the χ-uniform block mean square and the
   ...
   ⛔⛔ **WHAT IS NOT CLOSED AND IS NOT CLAIMED: THE ESTIMATE, AND ITS RANGE — the seat did not
   stamp, and the helm's 19:07 ruling above records that as right.** `M4ChiBlockMeanSqH h` asks for the χ-uniform block mean square at
   ...
   ***THE `h`-DOOR NEEDS EXACTLY THE `1`-DOOR'S CONDITIONAL, RESTATED OVER AN `h`-TIMES WIDER
   ...
   UNPRICED, and I did not price it by assumption.** The closure call was the helm's and it is
   made, above, in two halves.
   ...
   **no new kind of obligation**, meets **no cap anywhere in the corpus** (there is none to
   ...
   ⚖️⚖️ **HELM RULING 2026-08-31 19:07:09 — THE ROW'S STATUS, IN THE RULING'S OWN WORDS:**
   ***QUEUE item 7b is CLOSED AS TO THE CAP — the mint's `hsock` and `HDoorArc`'s socket both
   have kernel-checked producers at `h = 2` (PR #10 + #11) onto the SAME two data the `h = 1`
   door closes onto, no new gate, floor or constant — and its ESTIMATE-RANGE is an OPEN
   CONDITIONAL, named: `M4ChiBlockMeanSqH 2` (the χ mean square at `q ≤ 2·arcDen 12 H` against
   the landed `q ≤ arcDen 12 H`), owned by the PRODUCERS' lane, not the door's.***
...
8. **hb-engine OPENER** — the flagship's named campaign toward
   ...
   ⛔ **CORRECTION STAMP 2026-09-09 (h2c, Fable, the desk-row-HX sitting; established in freeze v1 §1 on 09/08, accepted by the helm 08:31 that day, re-confirmed by refuter R5 at the bytes, re-measured 09/09 at `f8e3424a`).** The parenthetical above — *"Wave A stopped at (7.5) on 08/11"* — is STALE and is left in place as the 09/03 stamp's own reading. Measured: Wave A landed (7.5) on 08/11 (`54939cf`) and (7.6)–(7.8) on 09/03 (`c9d1cdd2`; `Salt/HB/Lemma10Chain.lean`, 1,280 ln, 31 audited names, sorry 0), stopping at **the p.223 assembly of Lemma 10** by DECLARED decision (`Lemma10Chain.lean:27-31`: *"a sub-project of its own size, not a step; it is deliberately left to the next wave rather than half-built here"*). The A-seal freeze lineage on that assembly is at **v8** in the seat repo (`2026-09-09-h2c-FREEZE-N7-waveA-seal-v8.md`; the helm's non-author pass on v8 is owed). ⇒ A reader pricing N7 from this row would price a wave that closed six days ago. *(The 09/09 relight gate for row HX re-issued the stale premise verbatim: the correction had reached the bus and a bank but not the surface that issues orders — which is why it is stamped HERE, on the routing row, and not only posted.)*
   ✅⭐ **R10 LANDED — LEMMA 10 IS ON MAIN — 2026-09-09 22:54 PDT (h2c, Fable 5.1, one Opus executor per fire document; PR #100 the fifteen inputs, PR #102 the assembly, merged by the helm at `f0071bfa`).** `hb_lemma10` (`Salt/HB/Lemma10Seal.lean:1559`) — HB's Lemma 10 at the p.223 assembly, `K = sealK k = 2 + ⌊k^{1/4}⌋₊`, the frozen statement's md5 `545bc625a67a7afc5519b5fbfb307ff6` unmoved at the object — assembled from NINE R10 declarations (`majorant_tail_le` · `weighted_dyadic_block_sum_le` · `majorant_m1_le` · `fourier_column_le` · `majorant_head_le` · `majorant_rangeA_le` · `norm_majorantCoeff_neg` · `majorant_tsum_split` · `hb_lemma10`), each `[propext, Classical.choice, Quot.sound]`, sorry 0, native_decide 0, audited in `Salt/HB/All.lean` under `N7 WAVE A R10`; guarded 1245/1300 against the fire document's STOP; Lean Action CI + Scrub GREEN on the merged tree. The `m = 1` and `2 ≤ m ≤ K` rows are MIN-COMPOSED — the `K/m²` arm of (7.4) alone crossed the frozen `log(2k)²` budget near `k ≈ 1.3·10²⁴` (`flags.md` 2026-09-09, opened by the executor's kill on freeze v2 and RESOLVED by v4's re-cut in the same wave). Lineage, so the stamp above is read as history: seal freeze v0→v9 + erratum, with the helm's non-author passes on v8 (15 arms) and v9 (signed, `817986b3`) — **the "pass on v8 is owed" clause above is DISCHARGED** — then the R10 freeze v1→v4 (a 10-arm pass on v1; the signed v3 verdict 20:21, REPAIR-THEN-FIRE; v4 the fire document). ⇒ **§7 (LEMMA 10) IS CLOSED.** THE HONEST HEADLINE, re-cut: `N7Exit` = HB Lemma 5 = §7 ✅ + §5 (Lemmas 9/11) + §6 (the leading term); **§5 and §6 are UNTOUCHED**; the seal's instantiation rows `hb_lemma10_const` / `hb_lemma10_inv` / `hb_lemma10_road` are NOT landed and Wave B's consumer row B-6 decides which shape it needs; `hb_lemma10'` is RETIRED, never a statement (Wave B scout, item 5; `Lemma10Chain.lean`'s header corrected in this commit). THE FENCE STAYS (desk HX): no §5/§6 wave fires without the Captain's word; route freeze v2.3 §0′ was RULED 2026-09-11 17:30 (the `route-v2p3-s0prime` block DISCHARGED 09-12T01:10Z); the §5/§6 fence now stands under DECISION 4 — the Wave C fence word, desk HX — and lifts only on the Captain's word. Nothing here bears on twin primes.
   ⭐⭐ **WAVE W LANDED — `N7Exit` IS STATED AT HEATH-BROWN'S OWN LEMMA 5 INSTANCE — 2026-09-11 (h2c, Fable 5.1; the Captain's ruling at council lifted the N7 fence for this wave and only this wave; three Opus executors, one per fire document; every row on its first attempt).** PR #112 `678fdd6e` (the N8 wire's collapse as a kernel record: on `hbDataN8` every `S(d)`, `1 < d ∣ P`, vanishes and the Rosser sandwich is an identity — `lamSum_S_eq_S3`, `Salt/HB/CrownWireHB.lean`); PR #113 `ea0ce1e9` (`hbDataHB` — the pair `(4k+1, 4k+3)` at the k-range, HB's normalisation (1.3)–(1.9) at α = (4,4), β = (1,3); the bridge `hbDataHB_S3_le_S3_window` to the crown window at `X = 4x+1`; `hb_p200_lower_HB`; `hbDataHB_eq_hbDataForms_twin := rfl` to Wave B's forms wire); PR #114 `3d00e21b` (`N7Exit`'s data letter moved, binder unchanged; `hb_S3_at_hb_point` → `hb_S3_lower_at_hb_point`, one-sided; `hb_theorem1 : (1 − n9K/log ℓ′)·x𝔖C(4) ≤ S1 (Ioc (4x+1) (2(4x+1)))` — HB's Theorem 1 as a LOWER BOUND, the constant now the one pair's; `hb_theorem1_lower`; `crown_handover_k1`'s door at witness `4q^250+1` with `113 · 7232 · 7233²` from `𝔖 ≥ 1/28` exactly; the public docstrings restated with the honesty commit). **The crown's statement `heathBrownDichotomy_of_N7` is byte-unchanged; all ten names of the chain at `[propext, Classical.choice, Quot.sound]`.** Landed 695 insertions against the route's W cap 950–2,600. Design record: the route freeze v2.4 (§2 the diagnosis W1/W2/W3, §3 the five acts) and the Wave W freeze v1.1 (a seven-arm non-author pass, 0 KILL). What this does NOT do: `N7Exit` stays a hypothesis; the crown stays a dichotomy; nothing here bears on twin primes. **Next on this row: Wave C-2 produces `Lemma5Eval (hbDataHB …) 4 x …` — at HB's own data.**
   👑 **N9 EXECUTED WHOLE + THE LABEL RULED — 2026-09-04 19:1x (h2c, Fable; the Captain's word at the terminal).** `Salt/HB/CrownTheorem1.lean` on `h2c/crown-n9` (PR #37, 42/42 theorems at 3 axioms, 68 audit rows ✓, four Opus executors on a refuter-passed freeze v1→v3, zero flags) lands **the POLYLOG DICHOTOMY at exponent 30** (⚠️ **THAT EXPONENT IS THIS STAMP'S 09/04 READING; B3-i RE-GRADED IT TO `Poly 14` ON 2026-09-06** — the stamp below) — `heathBrownDichotomyPoly_of_N7 : N7Exit … → HeathBrownDichotomyPoly 30` = `TwinPrimeConjecture ∨ NoSiegelZerosPoly 30`, conditional on N7 — a NEW theorem (the effective polylog-Siegel bound is open) and an INTERMEDIATE, never called the crown in any public sentence (q ≥ 3; at q = 2 the polylog form is vacuously the stronger). THE CROWN'S STATEMENT stays the frozen `HeathBrownDichotomy` (the k = 1 member, byte-untouched; `heathBrownDichotomyPoly_one_iff`), reached by ARM B: **B0 ✓** (the `(a m b k)`-parametric T-BAL caps, PR #40 merged c673e995) · **B1a ✓** (the D–H contract at k = 1, `dh_repulsion_k1_of_floor` at (51, 12, 215, 1) — LANDED, `Salt/SW/TBalTall.lean:2971` on main, audited in `All.lean`) · **B2 ✓** (a log-free near-1 zero density by Jutila §2–§3 — **WHOLE ON MAIN 2026-09-06 05:22, `5f59e5ee`**, PR #71: `zeroCountM_density_logfree : ∃ C D, 0 < C ∧ 0 < D ∧ D ≤ 150 ∧ … N(σ,T,χ) ≤ C·(qT)^{D(1−σ)}` on `4/5 ≤ σ ≤ 1`, `T ≥ 2`, primitive `χ` mod `q ≥ 2`, at `Salt/SW/DensityStrip.lean:271`; sorry 0, native_decide 0, the three axioms, audited in `All.lean` under `⟦B2 W9f 0905⟧`; Lean Action CI + Scrub GREEN on the merged tree. Ruled 19:14 as class D at 4,500–7,500 lines by the scout; delivered in six sub-waves over two days with three hand-backs and ZERO statements false as spelled. B1b, the named FALLBACK, was never needed and is retired unspent) · **B3 ✓** (the row-(iv) tail RE-GRADE **and** the `k = 1` consumer — **WHOLE ON MAIN 2026-09-06 13:35, `4eb6d3d6`**; see the ARM B stamp immediately below) · **N7** (`N7Exit`, THE ONLY OPEN BINDER). The intermediate's q-coupling is the exponent k's and vanishes iff k ≤ 1, so the k ≤ 1 re-proof is the ONLY decoupling route; the `one_iff` row is the crown's exit test. Arm A's wiring (D3 + D5/D7) is not spent; what PR #37 carries of it is the intermediate's own plumbing. Nothing here bears on twin primes.
   👑👑 **ARM B IS WHOLE — THE CROWN'S FROZEN STATEMENT IS ON MAIN, ON ONE BINDER — 2026-09-06 13:35 (h2c, Fable 5.1 with three Opus executors; PR #75 `46127a34`, then PR #76 `4eb6d3d6`).** `Salt/HB/CrownTheorem1.lean:6999` — **`heathBrownDichotomy_of_N7 {Cerr CA CA' CC : ℝ} (hN7 : N7Exit Cerr CA CA' CC) : HeathBrownDichotomy`**; ⛔ **ANCHOR AND SPELLING AMENDED 2026-09-08 14:1x (h2c) — BOTH MOVED BY MY OWN PR #82, WHICH IS WHY THIS IS A REPAIR AND NOT A REQUEST.** The theorem MOVED `:6947` → **`:6999`** (the repair inserted +52 lines above it, at `:443`), and — the half a line number cannot show — **`N7Exit`'s STATEMENT CHANGED.** It was FALSE as spelled (vacuous at the principal character, and again at `q = 1`) and now binds HB Lemma 5's four hypotheses (`3 ≤ q` · `χ.IsPrimitive` · `3·log z ≤ log q` · `q^250 ≤ x ≤ q^500`), with the four sign facts hoisted OUT of the ∀ as an explicit first conjunct. ⚠️ **THE TYPE SIGNATURE PRINTED ABOVE IS UNCHANGED AND STILL EXACT** — which is the hazard, not the reassurance: a Wave C executor diffing the signature sees a match and concludes the binder did not move. ⇒ 📌 **A ROUTING ROW THAT PINS A THEOREM BY LINE AND SIGNATURE RECORDS NEITHER OF THE TWO THINGS THAT ACTUALLY MOVED.** The crown is still a CONDITIONAL and `N7Exit` is still the only open binder — the repair made that conditional NON-VACUOUS, deciding neither arm; sorry 0, native_decide 0, the three axioms, audited in `All.lean` under `⟦B3 0906⟧` (4 rows) · `⟦B3-i(a) 0906⟧` · `⟦B3-i(b) 0906⟧`. `Salt/TwinBar/SiegelTwin.lean` is **BYTE-UNTOUCHED** — `HeathBrownDichotomy` last moved at `2db34474`, long before Arm B — verified read-only by math at `dff0a2b6` and again on main at 13:37, so the statement that landed is the frozen one and not a restatement of it. **STAGE B3 IS TWO PARTS.** **B3-i** (PR #75) discharges **the row-(iv) tail RE-GRADE that the row above listed as open**: `Salt/HB/TailShells.lean` (13 declarations) cuts a SIBLING envelope, because `efEnvelope`'s two zero rows carry the crude count `137·(2T₀+5)·log(q(T₀+4))` **inside the definition** and so cannot be re-graded in place; it spends B2 shell by shell (`zeroSum_shells_le`, each zero indexed by `⌊log((1 − Re ρ)/w)/log(6/5)⌋₊`, the fibre sum closing at the constant `2`). `ellL` is gone and `logChiSum_tail_at_window` is widened by `n9Tail`. That re-grades the INTERMEDIATE **`Poly 30` → `Poly 14`** (`heathBrownDichotomyPoly_of_N7 : N7Exit … → HeathBrownDichotomyPoly 14`, **`:6981`** — ⚠️ *this pin was `:6929` in the very commit that repaired its sibling's `:6947`; the SAME paragraph carried a second stale line number and I did not check it. Re-measured at `origin/main` at the un-draft. A row that pins by line has as many stale-pin defects as it has pins.*). **B3** (PR #76) then sets `dhK := 1`, re-wires the D–H contract to B1a's `k = 1` sibling, un-couples `ℓ′` from `q`, and hands over through HB's OWN fulcrum (`FulcrumQualityMin`, not `FulcrumQualityPoly … 14`) — the supply's single `log L` cancelling the floor's `log(log 4q + 2)` EXACTLY; the bridge `fulcrumQualityMin_of_poly` keeps the `Poly 14` row alive at zero cost. ⇒ 🔑 **THE `k ≤ 1` RE-PROOF THAT THE ROW ABOVE NAMED AS “THE ONLY DECOUPLING ROUTE” WAS TAKEN, AND IT CLOSED** — the `one_iff` exit test it named is now discharged by an object, not by an argument. ⛔ **THE HONEST LABEL (the Captain's, unchanged in substance):** this is Heath-Brown's dichotomy `TwinPrimeConjecture ∨ NoSiegelZeros`, machine-checked, and **CONDITIONAL ON HB LEMMA 5 (`N7Exit`, still OPEN — four existential constants)**, on B2's `n9CB2`, and on Siegel's INEFFECTIVE threshold. **N7 IS NOW THE ONLY OPEN BINDER BETWEEN THIS PROGRAM AND THE DICHOTOMY.** ⛔ A DICHOTOMY IS NOT TWIN PRIMES: neither arm is decided, and nothing here bears on the twin-prime conjecture.
...
11. ⚖️ **W2-0 — THE TAO-THEOREM-1.2 ∀ε CAMPAIGN. SCHEDULED AT P3 BY THE CAPTAIN
   ...
   signature, so one `(x, ω)` serves every class. What S owes its own freeze: the S-PLATEAU
   argument (RENAMED from "S-4" 2026-09-14, desk `NI` — see the S-PLATEAU card in the Tier S block
   below) redone against the TWO builders that actually pin `Hhi`; the socket window's
   ...
   * ✅ **S-PLATEAU — OFF THE CRITICAL PATH** (Fable-tier, but not blocking): without it the road is `∃ᶠ N`, which
     D9 makes sufficient. It upgrades `∃ᶠ` to `∀`, it does not gate the terminal.
     ⚖️ **RENAMED FROM "S-4" 2026-09-14 22:0x BY THE HELM (63rd head), desk `NI`. THE OLD NAME IS KEPT IN THIS CARD ON
     PURPOSE** — a rename without a pointer does not remove a collision, it moves it into the history, where every bank,
     minute and bus post written before tonight still says "S-4" meaning THIS item.
     Until tonight two rungs shared the number and carried **opposite path claims**: this one (the PLATEAU, OFF the
     critical path) and **THE ENTROPY-ARROW BRIDGE** — the rung after S-3, frozen 2026-09-14 (`cac43839` → v1.1
     `8f0b1443`), desk `NE`, LANDED 21:0x — which is squarely **ON** it, being what S-3's band feeds. ⇒ 🔑 ***A READER
     WHO MET THE WRONG "S-4" FIRST GOT THE REASSURING ANSWER.***
     **THE BRIDGE KEEPS THE NUMBER AND THE PLATEAU TAKES A WORD**, and that direction is not a coin flip: the bridge's
     name is already load-bearing in a freeze, a desk row, a guard filename (`check_s4_statements.py`, which
     travels with the bridge's freeze in the private record), two Lean modules and two helm rulings, so renaming it would falsify receipts already pinned to shas — while the plateau's
     name lived in prose, in this one file. **A word rather than the next number** because the plateau is not a rung on
     the numeric ladder in the same sense: it upgrades a QUANTIFIER (`∃ᶠ` → `∀`) rather than supplying a link, so
     nothing in the `S-n` sequence can collide with it again.
     ⚠️ **AND THE NAME IS SPELLED OUT RATHER THAN ABBREVIATED, WHICH THIS EDIT LEARNED THE HARD WAY:** the first cut of
     this rename used the tag `S-P`, and the very next `grep` for it in THIS FILE returned a false positive —
     `WITNESS-PROOFING` contains the substring. ⇒ 🔑 ***A SHORT TAG IS A SUBSTRING OF ORDINARY PROSE, SO THE CENSUS THAT
     WOULD FIND FUTURE COLLISIONS IS THE FIRST THING IT BREAKS*** — the same substring law that makes `FS-4` answer a
     search for `S-4`. The name IS the words, which is also what this card asks every citation to do.
     ⛔ **AND THE COLLISION WAS WIDER THAN THE TWO — measured at `origin/main` while making this edit, and stated because
     the row that ordered the rename said TWO:** the bare token "S-4" ALSO names a **July-2026** item in this repo — the
     arc-floor page (`docs/blueprints/flags.md:13728,13761`; `docs/exploration/s11-spine-brief-0728.md:36`) and a ruled
     council item (`docs/exploration/council-0727.md:340,349`). Those are CLOSED HISTORY and are deliberately NOT
     renamed. ⇒ **Cite a Tier S rung by its WORDS — the plateau, the bridge — never by the bare number**, which has
     meant three different things in this corpus since July. *(`FS-4` in `Salt/MR/FarStar.lean` is a fourth token and a
     false positive: it is `FS-`-prefixed, and a substring search for `S-4` finds it. Recorded so the next census does
     not re-find it as a fifth collision.)*
   * ✅ **THE LADDER — LANDED 2026-09-14 23:1x. THE ITEM IS AT P1, NOT COPIED HERE** (the S-3 line's own
     precedent: pointed, not duplicated). Freeze v1.1 salt `bba88020` (seat `e2bd75d66`) · math's non-author NO KILL
     `687739ad7` · desk NU · 7/7 at `[propext, Classical.choice, Quot.sound]` · hah9 and P6 (the crown twin) carry
     the refuter's own construction (label, not gate) · the byte guard's hole (math's M9) closed on three arms.
     **It takes NO NUMBER** (`S-5` refused: a July council item and a July lemma carry it) — cite it by its words.
   * ✅ **S-4 (THE ENTROPY-ARROW BRIDGE) — LANDED 2026-09-14 21:0x. THE ITEM IS AT P1, NOT COPIED HERE** (the
     S-3 line's own precedent: pointed, not duplicated). Freeze v1.1 (seat `aa9858f91`) · math's non-author NO KILL
     `853b51201` · desk NE · 8/8 at `[propext, Classical.choice, Quot.sound]` · B and K2′ carry the refuter's own
     construction (label, not gate).
