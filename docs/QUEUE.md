...
✅ **ROAD F · W-δ — LANDED 2026-09-18 (h2c; PR #156 → merge `8dd67415` on `main`), AND THE SECTION ABOVE SAYS "NOTHING IS LANDED".** That sentence was TRUE when it was written (09-17) and is SUPERSEDED HERE; it is left in place as its own dated record. ⛔ **THIS ROW IS THE SAME FIX AS THE RUNG-2 ROW ABOVE, FOR THE SAME DEFECT, A SECOND TIME:** measured at `origin/main` `046acf6b` on 09-19, `FlatDoorAllGrades` · `flatDoorAllGradesW_holds` · `FlatDoorNonVacuity` · `#156` · `#157` · `8dd67415` each scored **0** in this file against live controls (`rung 2` 1 · `crownNV_landedW_nonvacuous` 1) — the landing sat on `main` for ~22 hours routed nowhere.
  ...
  - ⚠️ **NV-c IS NO LONGER "PRICED, NOT DRIVEN": IT IS DRIVEN AND UP AS A DRAFT (#158, head `827dd058`, 2026-09-19), NOT LANDED.** `Salt/MR/FlatDoorParityFloor.lean`, 12 theorems: the `ξ = 0` term of the door is `≥ 1/H²` at odd `H`; EVERY `ChowlaRegime` has `Hlo + 1 ≤ Hhi`; a regime with `0 ∈ Ξ_H` on its range carries the door only at `ρ ≥ 1/(Hlo+1)²`; on the flat family above the non-vacuity floors `∃ R ∀ ρ` is FALSE (K6b, in the honest form v1.1 named). The price was 6 declarations with an integrability risk; the risk did not exist (a landed one-liner) and the monotonicity lemma whose needle returned 0 on 09-17 IS landed (`chowlaTower_monotone`). ⛔ **DRAFT ON PURPOSE:** two of its statements are new, and the non-author pass is routed to math with kill-checks pre-registered. **The refutation over ALL regimes is NOT claimed** — the Mertens floor is non-effective.
  ...
  - **NEXT, and it is h2c's:** #158's non-author verdict → un-draft → merge; then this row's NV-c bullet is re-stamped LANDED in the same act. Beyond that road F has NO commissioned hop: the next hop toward the crown is a statement, and a statement is the Captain's word at a sitting.
