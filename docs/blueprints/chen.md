...
| C1c‴ | **discharge `hTbound`** — the Lemma 11 analytic induction: per-Tₙ chain-to-integral (the (34) recursion via hyp (4) + Abel/partial summation, chain sums vs `∫ fseq(t−1)`), the four term-bounds (35)–(38) against hbar (then `hbar_le_hBJS`), the (39) odd small-s case, and the Lemma-12 τ-close (`Στ_{2n−1} ≤ C₁`, `Στ_{2n} ≤ C₂`; parametric + the frozen-row instantiation). A genuine large node — its own session | C+ |
