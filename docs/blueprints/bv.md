...
| V3.1 | **ψ-form BV at fixed scales** (RE-RE-FROZEN, 6th correction — the y-sup STRUCK; ∀ fixed y ≤ x with y-uniform constants): `SiegelWalfisz → ∀ A > 0, ∃ B C, ∀ x ≥ 2, Σ_{q ≤ ⌊√x/(log x)^B⌋} maxAP-discrepancy(y, q) ≤ C·x/(log x)^A` for EVERY y ≤ x (the scale-net max is V4's, via crude increments) — combining V1a (per-y) + V3.0 + conductor descent + V3.2 (small conductors) + V2a/V2b dyadic (large conductors) + the Vaughan head. Vaughan parameters **`U = V = x^{1/10}`** (fixed by the V2a freeze — the `x^{1/5}` original breaks TypeI₂; window `θ ∈ (0,1/8)`). THE keystone assembly. — LANDED PB-floor (`Salt/BV/Dispersion.lean`, 900 lines): ALL the number theory proven (`regroupL1_perq`, `dispSum_le`, `largeEnergy_le`/`smallEnergy_le`, `vaughan_bridge` defeq, `dispSum_split_le`, `absorb_real/nat`, `smallY_bound`, the branch assembly); ONE deferred hypothesis `hlargeY` = the pure-rpow term bounding of the explicit split RHS (Mc = 26+16384√2, Kc = 262144+8K·2^{2A+7}; budget-verified, ~120 lines) → node V3.1-close: CLOSED — `largeY_bound` + **`psi_BV_of_siegelWalfisz'`** (`Salt/BV/DispersionClose.lean`, 445 lines; per-term budget hits Mc = 26+16384√2, Kc = 262144+8K·2^{2A+7} exactly; SW alignment RE-TUNED to C_sw = 2A+12/A_sw = 4A+19 needing only log x ≥ 4 — the flag recipe's C_sw = A+7 needed an unavailable log x ≥ 2^{A+7}). THE ψ-BV KEYSTONE IS UNCONDITIONAL MODULO THE GATE. | C+ | ✅ |
...
| V4.2 | θ→π: partial summation `π(x;q,a) − π-main = [θ-discrepancy at x]/log x + ∫₂ˣ [θ-discrepancy at t]/(t log²t) dt` — **consumes V3.1's ∀-fixed-y form on a polylog net of scales + the crude within-shell increment bound** (6th correction; increments via the trivial `width/q + 1` count — Σ_q max_a ≤ L(width·L + Q), no Brun–Titchmarsh) (adversarial pass: the ∫ needs the discrepancy at ALL scales; `∫₂ˣ dt/(t log²t) ≤ 1/log 2` then closes it; the 'cruder sandwich' idea is FALSE — it destroys the main-term cancellation). Plus the `maxDiscrepancy` (sup')-packaging over reduced residues. `Salt/BV/PsiToPi.lean`. | C | ⬜ |
