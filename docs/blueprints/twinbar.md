...
| T4 | `J_bounds : J₁ F ≤ log 2·(iterated ∬ w₁F²)` and the J₂ mirror + the ONE Tonelli swap aligning orders — the `lintegral`-first lever: do (iii)–(v) in `ℝ≥0∞` where Tonelli is side-condition-free (`lintegral_prod` needs only AEMeasurable), descend to ℝ once at the end via nonnegativity | **C** | ⬜ |
