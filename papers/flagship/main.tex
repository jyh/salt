...
% ============================================================================
% Journal class: Forum of Mathematics (CUP-JNL-FMP.cls, from the official
% author template). The compliance re-cut of 2026-08-18 changed packaging
% only; no claim sentence was altered (receipts in the commit message).
...
% Engine guard (compliance re-cut): pdfLaTeX (arXiv/CUP) uses the original
% \DeclareUnicodeCharacter mappings; XeTeX (local tectonic builds) maps the
% same glyphs via newunicodechar. Identical rendering path, per-engine syntax.
\ifdefined\XeTeXrevision
\usepackage{newunicodechar}
\newunicodechar{¬}{\lchar{\neg}}
\newunicodechar{Λ}{\lchar{\Lambda}}
\newunicodechar{α}{\lchar{\alpha}}
\newunicodechar{β}{\lchar{\beta}}
\newunicodechar{δ}{\lchar{\delta}}
\newunicodechar{ε}{\lchar{\varepsilon}}
\newunicodechar{θ}{\lchar{\theta}}
\newunicodechar{μ}{\lchar{\mu}}
\newunicodechar{ξ}{\lchar{\xi}}
\newunicodechar{ρ}{\lchar{\rho}}
\newunicodechar{χ}{\lchar{\chi}}
\newunicodechar{ω}{\lchar{\omega}}
\newunicodechar{‖}{\lchar{\Vert}}
\newunicodechar{₀}{\lchar{{}_0}}
\newunicodechar{₊}{\lchar{{}_+}}
\newunicodechar{ℂ}{\lchar{\mathbb{C}}}
\newunicodechar{ℕ}{\lchar{\mathbb{N}}}
\newunicodechar{ℚ}{\lchar{\mathbb{Q}}}
\newunicodechar{ℝ}{\lchar{\mathbb{R}}}
\newunicodechar{ℤ}{\lchar{\mathbb{Z}}}
\newunicodechar{→}{\lchar{\to}}
\newunicodechar{∀}{\lchar{\forall}}
\newunicodechar{∂}{\lchar{\partial}}
\newunicodechar{∃}{\lchar{\exists}}
\newunicodechar{∈}{\lchar{\in}}
\newunicodechar{∏}{\lchar{\prod}}
\newunicodechar{∑}{\lchar{\sum}}
\newunicodechar{∣}{\lchar{\mid}}
\newunicodechar{∧}{\lchar{\wedge}}
\newunicodechar{∨}{\lchar{\vee}}
\newunicodechar{∫}{\lchar{\int}}
\newunicodechar{≠}{\lchar{\neq}}
\newunicodechar{≤}{\lchar{\le}}
\newunicodechar{≥}{\lchar{\ge}}
\newunicodechar{⌊}{\lchar{\lfloor}}
\newunicodechar{⌋}{\lchar{\rfloor}}
\else
...
\jyear{2026}
% The class's \copytext default hardcodes 2020; reissue its own sentence with
% the submission year. (Caught by the Captain's read of the built PDF.)
\copytext{\textcopyright\ The Author(s) 2026. This is an Open Access article,
...
\title[Twin Primes and Siegel Zeros]{Twin Primes and Siegel Zeros: a Fulcrum}

\author[1]{Jason Hickey}\orcid{0000-0003-0300-021X}

\authormark{Jason Hickey}

\address[1]{\orgname{Independent researcher}}

\abstract{Every theorem in this paper is verified by the Lean~4 kernel over
\textsf{mathlib}, with axiom base exactly
$\{\texttt{propext}, \texttt{Classical.choice}, \texttt{Quot.sound}\}$;
nothing rests on informal argument. We introduce the \emph{fulcrum}: a
single hypothesis---one fixed quality constant $C^\star$, one zero per
unbounded window---under which, together with an explicitly stated engine
hypothesis, Siegel zeros produce infinitely many twin primes; the zero's
reality is derived rather than assumed, and the fulcrum is proved minimal
for the engine that consumes it. Around it we prove kernel-checked
zero-free regions for $\zeta$, a Deuring--Heilbronn repulsion contract, an
exchange-rate neutrality theorem pricing transfer-based routes to twins,
and the parity gap as a mathematical object. The corpus is $324{,}724$
lines of Lean across $751$ files, produced by the \emph{Salt method},
which is described as a contribution in its own right.}

\keywords{twin primes, Siegel zeros, zero-free regions, parity problem, Chowla conjecture, formalization of mathematics, Lean theorem prover}

\keywords[MSC Codes]{\codes[Primary]{11M26}; \codes[Secondary]{68V20, 11N35}}

\end{Frontmatter}

% ---------------------------------------------------------------------------
% Overview (journal-required, 350--500 words). The text from the second
% paragraph onward is the pre-recut abstract, MOVED VERBATIM -- every
% priority-sensitive sentence and its dated footnote are byte-identical to
% the frozen tip. Only the opening context paragraph is new.
% ---------------------------------------------------------------------------
\section*{Overview}

Heath-Brown proved \cite{HB1983} that infinitely many Siegel zeros would
force infinitely many twin primes; the zero-free-region landscape such
arguments inhabit descends from Littlewood \cite{Littlewood1922}.
Machine-checked mathematics at research scale has its landmark in the
Kepler verification \cite{HalesKepler}, over substrates such as
\textsf{mathlib} \cite{mathlib}. This overview places the paper in that
context and states its results; the abstract above is a summary.
...
engine-conditional horn stated explicitly; the first formalization, in any
public artifact as of 2026-08-17,\footnote{Priority statements in this paper are \emph{surveyed as-of} claims, not assertions of absence. Surveys: \texttt{docs/priority-survey-2026-08-11.md} and the pre-submission re-run \texttt{docs/priority-survey-2026-08-17-delta.md}, both in this repository (method: adversary-cited, five independent lanes each; two of the re-run's changed verdicts rest on our own local kernel checks of competitors' public bytes, not on their claims). This repository is public as of 2026-08-16; the earliest third-party-verifiable date for its contents is 2026-08-14, the submission timestamp of the companion methods paper; internal commit dates before that are reported as internal records, never as public priority. Our own landing dates are given in \S\ref{sec:banners} and the results register; the survey re-runs once more at submission.} of any zero-free region for
$\zeta$ of Littlewood strength (1922) and the first such power-saving region
($\theta = 3/4$) in any proof assistant; a
...
twin-sufficient, parity-invariant completion exists, together with a
census placing ten headline theorems of the corpus inside the
parity-invariant cone. As a
...
that process is the weakest twin-producing Siegel hypothesis in the
surveyed record (Definition~\ref{def:fulcrum}); the process itself,
\emph{demand-audited hypothesis minimization}, is---we
argue---a new mode of mathematical discovery, and it is the sense in which this paper's
formalization is an instrument rather than a notary.

The results divide into four movements. First, the \emph{banners}
(\S\ref{sec:banners}): unconditional analytic theorems, two of them
formalized for the first time in any public artifact as of 2026-08-17: a
Littlewood-strength zero-free region for $\zeta$ and the first such
power-saving region in any proof assistant. (The Vinogradov mean value
theorem that feeds the latter carries its own dated qualification, stated
at Remark~\ref{rem:vmvtdate}.) Second, the
...
epistemic status. Verification at this scale is not itself new---the
formal proof of the Kepler conjecture \cite{HalesKepler} is the standing
landmark---but the object here is a research program in progress rather
than a settled theorem, which is what makes the demand audit of
\S\ref{sec:fulcrum} possible at all.
...
Four unconditional theorems anchor the corpus. We do not assert that they
were unformalized---an absence no repository search can establish, as the
survey behind this paper's priority statements found to its cost, twice---and
cite the adversaries instead: the canonical public Lean formalization of
Bombieri--Vinogradov proves its main theorem modulo a Siegel--Walfisz
statement \emph{axiomatized} in its own source, and says so; its companion
large-sieve input, recorded as an axiom by the 2026-08-11 survey, is
discharged by a proof (primitive-character scope, Gallagher's route) public
since 2026-07-09 as that repository's first pull request, merged
2026-08-04---which the 2026-08-17 re-run kernel-checked clean from its
public bytes, and which therefore precedes this repository's public dates.
The two proofs live on sibling branches, not yet merged. As of 2026-08-17 no
public artifact other than the present corpus proves unconditional
Bombieri--Vinogradov; Siegel--Walfisz is the wall---a declared axiom on all
four branches of the canonical repository and in a second project, a
\texttt{sorry} or a self-described scaffold in the others the survey read.
...
\begin{theorem}[A power-saving region, $\theta = 3/4$]\label{thm:pow}
There exist effective constants such that $\zeta(s) \neq 0$ for
...
As of the 2026-08-17 survey re-run, Theorem~\ref{thm:pow} is the first zero-free
region with a power saving in the exponent verified in any public
proof-assistant artifact, and Theorem~\ref{thm:litt} the first formalization
of the 1922 Littlewood region \cite{Littlewood1922}; the strongest previously formalized regions were of classical
de la Vall\'ee Poussin shape. Our exponent convention for zero-free
regions, stated so that the
claim is checkable: $\theta$ is the power of $\log|t|$ in the denominator of
the zero-free width, so the de la Vall\'ee Poussin region
$\sigma \ge 1 - c/\log|t|$ sits at $\theta = 1$, the Vinogradov--Korobov
region of the prose literature
$\sigma \ge 1 - c\,(\log|t|)^{-2/3}(\log\log|t|)^{-1/3}$ at $\theta = 2/3$,
and \emph{power saving} means $\theta < 1$. Theorem~\ref{thm:pow} lands at
$\theta = 3/4$: strictly between de la Vall\'ee Poussin and the classical
$2/3$, which on the same survey no public proof-assistant artifact attains;
we do not claim the $2/3$ exponent. The proofs formalize the Vinogradov mean
...
\begin{remark}[A dated qualification]\label{rem:vmvtdate}
For this theorem the as-of dating requires more than the survey's default
as-of form---a dated adversary citation: the \texttt{lean-lang.org}
evaluation board records its Vinogradov-mean-value problem---stated there
in full main-conjecture form---as solved on 2026-08-16 by a third-party
submission (an automated system),
verified by the board's own same-statement comparator (kernel-accepted,
axioms within the standard three), with the solution source private and its
publication announced for 2026-08-31. No public artifact proves the theorem
as of 2026-08-17, but a kernel-grade, dated counter-attestation exists and
its artifact is expected to be public before review of this paper
completes. We therefore attach no priority weight to
Theorem~\ref{thm:vmvt}: it appears here as the input that
Theorem~\ref{thm:pow} consumes, and the survey delta records the board
entry as the adversary citation.
\end{remark}
...
Write $L = \log(2x+2)$. Let $\chi$ be a Dirichlet character with
$\chi^2 = 1$, let $100^{16} \le z$ and $L^8 \le z$, let $z^3 \le x$, and let
$W$ be the sifted window at parameters $(z,x)$. Then
\[
S_2(\chi, W) - S_1(W) \;\le\; 2^{31}\Bigl(\frac{x}{z_0}
+ \frac{x}{\log x}\,e^{5 z_0}\,\mathrm{PS}_\chi
+ e^{2 z_0}\bigl(x z^{-1/8} + x^{9/10}\bigr) L^3\Bigr),
\]
where $z_0 = \log(2x+2)/\log z$ and
...
functions (60 declarations: the \leaninline{IsSignFunction} structure, 11
definitions, and 48 proved statements) required deleting every coprimality
thread;
that the deletion is sound is itself part of the formal content: the
character was never necessary.
...
For $0 < \theta < \tfrac12$ and $A_0 \le 2$, no predicate $E$ is both
twin-sufficient and parity-invariant. The witness is the twin-free weighting
...
bound $J_1 + J_2 \le 2\log 2 \cdot I_2$ \cite{Maynard2015} and its $\theta$-parameterised
...
infinitely many Siegel zeros; Chen's theorem \cite{Chen1973} (infinitely many primes $p$
...
The three results triangulate the frontier exactly: the corpus's censused
headline theorems live inside the cone (Proposition~\ref{prop:census}); the twin
...
Matom\"aki--Radziwi\l\l--Tao's Proposition~2.4 \cite{MRT} provides in prose
...
Matom\"aki--Radziwi\l\l\ program \cite{MR2016} (whose first stones, including the
...
$6(1 + \log K)$.
\leanname{Salt.Weil.sawtoothMajorant\_fourier\_expansion,\allowbreak
Salt.Weil.norm\_majorantCoeff\_le\_sq,\allowbreak Salt.Weil.tsum\_norm\_majorantCoeff\_le}
...
Thm~\ref{thm:wall} & the overshoot budget (\texttt{hbudget}) & \S\ref{sec:walls} \\
\bottomrule
\end{tabular}
...
Thm~\ref{thm:rep} & \leaninline{dh\_repulsion\_tall} & the three \\
...
(2026-08-03), except for a minority of entries re-pinned at later commits;
every pin re-extracts at one fresh commit at the release re-extraction, and
the source is the arbiter wherever this rendering and the declaration could
be read apart.
...
contract form). Source \leaninline{dh\_repulsion\_tall} at
\leaninline{Salt/SW/TBalTall.lean:2090}; audit
\leaninline{Salt/SW/All.lean:132}, the track's roll-call invocation, which names
this declaration at \leaninline{:346}. The three constants are bound
existentially in the statement; the witnesses are supplied at
\leaninline{Salt/SW/TBalTall.lean:2194} as $\langle 680, c, 14, \ldots \rangle$,
with $c$ the explicit ten-arm minimum set at \leaninline{:2137}. The paper's
$b = 680$ and $k = 14$ are therefore proof data, not statement data, and a
reader checking the display of \S\ref{sec:banners} against the declaration
should read them there. The height-bounded twin of
Remark~\ref{rem:repc} is \leaninline{Salt/SW/TBalR8.lean:1813} (witness
\leaninline{:1921}, minimum \leaninline{:1861}): the same $b$ and $k$ over an
eleven-arm tower whose extra arm carries the auxiliary $\zeta$-growth constant,
which is why that form does not price numerically and this one does.
{\footnotesize\begin{alltt}
...
\leaninline{Salt/HB/All.lean:71}. The modulus is supplied by the section
...
(the rate); audit \leaninline{Salt/HB/All.lean:71}. The split the paper states is
...
\leaninline{Salt/HB/All.lean:71}. Four statements, four lines;
...
\leaninline{Salt/Parity/Z.lean:678}; audit \leaninline{Salt/Parity/All.lean:22}.
The grade condition is the single binder \leaninline{h2}: $A_0 \le 2$. Until
2026-08-11 this theorem also carried \leaninline{h1} ($1 \le A_0$) and
\leaninline{ht} ($E(\mathbf{1})$); writing its comprehensibility certificate showed
both were unused by the proof, and they were dropped --- the kernel had been holding
a strictly stronger statement than the one displayed.
{\footnotesize\begin{alltt}
theorem sufficient_true_not_parityInv \{θ A₀ : ℝ\} (h0 : 0 < θ) (h : θ < 1 / 2)
    (h2 : A₀ ≤ 2) \{E\}
    (hs : TwinSufficient θ A₀ E) : ¬ ParityInv θ A₀ E
\end{alltt}}
...
% --- Journal-required statements (template backmatter form) ----------------
\paragraph{Funding statement}
This work received no external funding.

\paragraph{Competing interests}
The author declares none. The capacity and employer-independence
disclosures in the Acknowledgments apply.

\paragraph{Data availability statement}
The complete Lean~4 corpus underlying every theorem, the commit ledger,
and the priority surveys cited in the text are publicly available at
\url{https://github.com/jyh/salt}; Appendix~A maps each stated result to
its Lean declaration and axiom audit.

\paragraph{Author contributions}
Developed in collaboration with Claude (Anthropic's Fable~5 and
