...
single named hypothesis. The corpus is $324{,}724$ lines of Lean across
$751$ files; the method that produced it---the \emph{Salt method}---kept a
public ledger of every design error caught (255 numbered entries) against
...
at 255 catches against zero wrong proofs and is offered as evidence that the standard of the first
...
classical refereeing process. One consequence for the reader: every
...
There are explicit constants $b = 680$, $k = 14$, $c > 0$ (with $c = c_1^8$
for an explicit $c_1$) such that: for every primitive quadratic character
$\chi \bmod q$, $q \ge 2$, every real zero $\beta_0 \in (\tfrac12, 1)$ of
$L(s,\chi)$, and every non-real zero $\rho$ of $L(s,\chi)$ with
$|\mathrm{Im}\,\rho| \le 1$ and $\tfrac{16}{17} \le \mathrm{Re}\,\rho \le
\beta_0$,
\[
1 - \beta_0 \;\ge\;
...
Nine headline theorems of the corpus lie inside the parity-invariant
...
with $p+2$ a $P_2$); the twin almost-prime bound $\Omega(n(n+2)) \le 20$
infinitely often; Brun's theorem (the twin reciprocal sum converges);
and the Selberg twin counting bound $\pi_2(x) = O(x/\log^2 x)$.
\leanname{Salt/Parity/Instances.lean (nine declarations, one per
theorem)}
\end{proposition}
...
standard one,
\[
\frac{1}{\log x} \sum_{n \le x} \frac{\lambda(n)\,\lambda(n+2)}{n}
\;\longrightarrow\; 0,
\]
...
classifies the components; executor models (here Claude Opus) prove
them at their tier. Misclassification is safe rather than fatal: an
