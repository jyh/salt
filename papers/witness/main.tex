...
\leanname{Salt.Entropy.Chowla.ChowlaRegime (Salt/Entropy/Chowla/Regime.lean:56)}
...
\texttt{Salt/Entropy/Chowla/TowerFlatRegime.lean:122}); it is the structure
...
(Salt/Entropy/Chowla/TowerFlatBuilder.lean:95)}
...
(\leaninline{Salt/MR/S16FlatTerminalLinear.lean:49}) is the band lane's
constant hypothesis. Everything in it beyond a landed unconditional theorem
(\leaninline{Salt.MR.mmuChiRate\_holds\_gated}, carried through
...
is not payable as stated. \S\ref{sec:limit} records what happened to it.
...
$\mathrm{S16BaseScaleCap96\_L\_gk}$, the base-scale cap
\[
...
\]
\leanname{Salt.MR.kcap, Salt.MR.KlevF (Salt/MR/KLever.lean:147)}
...
and with the two 2~August repairs in place.
...
\lnot\,\mathrm{logChowla2Fails}(R.\varepsilon, R.x, R.\omega).
\]
\leanname{Salt.MR.logChowla2\_witnessed\_scale\_flat\_L\_v2\_uniform\_win\_%
xceil\_cqhoist (Salt/MR/RegisterCompose.lean:209)}
\end{theorem}
...
so the route died and the object it fed did not: Step 5's statement is
...
\begin{itemize}
\item $e^{-100} \le K_s$. A fixed numeral against a fixed constant whose
  content is Siegel-ineffective: nothing in the corpus or in the literature
  bounds it from the side the rider needs.
...
Appendix~\ref{app:signatures}. Full index generated at release.}
\label{tab:index}
\end{table}
...
Thm~\ref{thm:pearl} & band-lane constant ($\log C \le 40$); & landed;
  cap arm false \\
 & 8 numeric riders; 2 carried predicates & at the produced regime \\
Thm~\ref{thm:pearl-recut} & band-lane $A$-window; 5 riders; &
  landed \\
...
\leaninline{Salt/MR/S16FlatFinal.lean:136}; audit
\leaninline{Salt/MR/All.lean:7195}. The eight riders are the eight arrows of
...
\leaninline{Salt/MR/RegisterCompose.lean:345}; audit
\leaninline{Salt/MR/All.lean:7736}. There is no hypothesis to the left of the
