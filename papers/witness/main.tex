...
not, and we say so in the next reading.

\emph{Third: the extremality question, and its exact present status.} It
is natural to ask whether some third threshold shape evades the toll
entirely. An exploratory pass over that question (nineteen probes, recorded
in the repository's ledger) found the flat tower extremal --- no shape
beats it by more than a factor $2$ in the exponent base --- and closed
several escape routes, including cheap levels and $Q$-ary steps. That pass
produced kernel-elaborated probes, not a landed corpus theorem, and this
paper accordingly claims no shape-free statement. We record the question as
open in the precise form: is there a decrement step whose concatenation
error is not of the form $\varepsilon^{2}/m$, and does it evade
\eqref{eq:bracket}?
%
% TODO(extremality): if the shape-free width law is landed as a corpus
% theorem before submission, promote this paragraph to a theorem with its
% own \leanname and move the probe record to \S\ref{sec:method}. Until
% then it stays a stated open question -- the claim is NOT in the kernel.
%
